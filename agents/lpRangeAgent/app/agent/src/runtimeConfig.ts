...
/**
 * The reference analyzer has no application-layer authenticator. Refuse
 * metadata-only auth configuration until an enforcing middleware exists.
 */
export function assertUnauthenticatedAnalyzerConfiguration(
  ...
  if (configured.length > 0) {
    throw new Error(
      `Unsupported authentication advertisement configuration: ${configured.join(", ")}. ` +
        "The LP analyzer does not advertise authentication until enforcement exists."
    );
  }
}

/** Resolve only bounded, non-secret transport configuration. */
