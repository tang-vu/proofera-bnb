...
/**
 * M1 is intentionally unauthenticated at the application layer. Refuse any
 * configuration that would imply authentication until the server enforces it.
 */
export function assertUnauthenticatedM1Configuration(
  ...
  if (configured.length > 0) {
    throw new Error(
      `Unsupported authentication advertisement configuration: ${configured.join(", ")}. ` +
        "ProofEra M1 does not advertise authentication until enforcement exists."
    );
  }
}
