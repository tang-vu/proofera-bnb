...
Current TTY policy admission uses v3 challenge/frame domains over the nonce-bound, bounded ASCII `BEGIN`/`CHUNK`/`END` state machine introduced in v2. Every line binds the challenge nonce, exact line index, chunk count, policy byte length, and lowercase raw policy-byte SHA-256; each `CHUNK` also binds its exact chunk index. Lines must arrive in exact order with no duplicate, gap, blank/control/trailing data, using only LF or CRLF. The reader retains a `4,096`-byte hard line cap, while valid worst-case construction has an exact `2,618`-byte maximum content line and a conservative tested bound of `2,700` bytes. The whole transport is at most `102,400` bytes and the policy is `1..65,536` bytes. Every non-final chunk carries exactly `2,304` base64url characters, `N` is derived from the canonical unpadded-base64url length, and `N` cannot exceed `38`. The historical `11,560`-character policy would occupy six chunks and eight total lines under this bounded layout; that arithmetic does not reauthorize it. A one-off local Windows Console observation motivated these conservative constants, but its harness and result are not retained and it is not reproducible repository evidence. Repository unit tests establish only the transport arithmetic and parser behavior. One absolute five-minute deadline covers the entire frame. The decoder reconstructs the bytes once, rechecks canonical base64url, declared length, and SHA-256, and rejects retired v1/v2 domains, truncation, reorder, missing/duplicate chunks, bad terminators, or buffered trailing input before policy admission, fresh RPC, custody access, or owner authority. These checks do not prove that the OS input queue had no earlier data and do not provide preload or malicious-same-user protection.
...
The pre-recovery runtime separated four timing bounds. The unsigned coordinator envelope lasted exactly `300` seconds. Owner entry was allowed for at most `240` seconds, with the strict deadline `min(challengeIssuedAt + 240 seconds, envelopeExpiresAt - 45 seconds)` so the entire `45`-second execution-authority lifetime remained after confirmation. The v4 owner text and exact-byte-confirmation frame bound `challengeIssuedAt`, `confirmationNotAfter`, the exact 45-second lifetime, and the deterministic derivation rule; they could not bind future clock values. Only after an exact byte match did the internal clock capture actual `confirmedAt`, and the `WeakMap`-branded v4 command bound that value and `executionExpiresAt = confirmedAt + 45 seconds`. A post-claim dual-RPC capability could refresh its observation/authentication timestamp to a value at or after `confirmedAt`, had to keep that same execution expiry, and was usable only while the recheck was at most `30` seconds old. Generation 2 retains these timing limits but binds its predecessor fence and attempt identity through owner-v5 domains. These clocks remain separate from the unchanged five-minute policy-frame deadline and numeric transport limits; generation 2 bumps the challenge/frame domains from v2 to v3.

The next one-off local operational exercise used audited release commit `336af2967286795dc7703fff85034c71b8e84b5c`, tree `86cc383388982dac1a2bea430f54d54e56bb6cf9`, runtime-manifest SHA-256 `0xa1cda6fcf00f8a7d2b9a679cfb9b3fc28aa60674dae89c7dbfc032bdbcff5bdd`, and canonical policy digest `0x8ddae3b13ee64ff5f983ce30d06c84d671e0a0ca029f75b5482de6b34b18ba54`. The `8,826` canonical policy bytes had raw SHA-256 `0x87de481e35a0d8fe6c503f4a7c832d665699ac9f20aa79eb5c40471d79e71a45`, encoded to `11,768` unpadded-base64url characters, and were admitted as six chunks. The owner entered the exact v4 confirmation; the legacy signing journal then durably wrote only slot 1. Windows QuickEdit/selection froze the console while the post-claim RPC recheck was pending. After selection was cancelled after the 45-second authority expired, the observed result was `POST_CLAIM_RECHECK_OUTCOME_UNKNOWN` without a transaction hash. Exact journal inspection and code ordering show no `worker_authorized`, `worker_started`, signed, or terminal slot; the submission journal was exactly empty, and the operation did not open custody artifacts, invoke DPAPI, unlock custody, sign, send, receive a receipt, create the pool, or add liquidity. The raw console output and operational harness were not retained, so the owner/terminal observation is not reproducible repository evidence or a release gate. A later live dual-RPC check observed nonce `1`, an absent factory pool, empty candidate code with nonce `0`, and the expected simulation result, but that check and its harness were not retained; it is a one-off no-effect observation, not reusable evidence or authority. The v4 confirmation and release policy are expired historical inputs and cannot authorize changed recovery code.

Recovery generation 2 is deliberately incident-specific. It recognizes only the exact `1,123`-byte legacy slot-1 claim whose raw SHA-256 is `0xf10e90eb836a94446ace100bbc9a6fc5de6cc35b1d82e4d10fb4736ef8559e32`, after its authority expiry, with no later legacy slot and exact-empty generation-2 signing and submission state. Invocation A may obtain a fresh read-only dual-RPC no-effect snapshot A, immediately reread all three durable namespaces, append an `O_EXCL`/fsync/self-hashed `superseded_before_worker` fence to legacy slot 2, reread it, and then must stop with `PREDECESSOR_FENCE_RECORDED_RESTART_REQUIRED`; it cannot reach release policy, owner TTY, custody, signing, or submission. Only a separate invocation B may retain that exact fence, admit a fresh committed-and-audited release policy through TTY v3, obtain a distinct execution snapshot B strictly after the fence, instantiate that policy on B, and request a fresh owner-v5 confirmation for a generation-2 attempt. Any worker authorization/start, signature, submission content, partial or mixed namespace, path/ACL/file ambiguity, race, or unknown append result blocks recovery rather than becoming a generic retry. Generation 2 uses a separate `bsc-testnet-pta-wbnb-pool-v2` signing namespace and binds `{ generation, predecessorState, predecessorFenceSha256, attemptId }` end to end. The existing `bsc-testnet-pta-wbnb-pool-submission-v2` directory is retained visibly, while its generation-2 journal schema and files are v3.

A separate submission/reconciliation stack validates the one exact generation-2 signed transaction, fresh dual-RPC pre-submission evidence, the recovery quartet, a v3 durable owner-authorization/submission journal, restart-first recovery, winner-only durable `submission_started`, and a second terminal dual-RPC reread after that durable acknowledgement and immediately before the single private send attempt. Ambiguous or restarted outcomes reconcile without resend or replacement. For each fixed provider, terminal confirmation uses the provider-attested fixed receipt-plus-128 sandwich `F1 -> C1/ancestry -> C2 -> F2 -> EIP-1898(C)`, plus receipt-block EIP-1898 post-state/logs. The finalized heads may differ, but both providers must agree on the checkpoint, ancestry, canonical attestation, and receipt-block post-state/logs. This is bounded dual-provider RPC consistency evidence, not a cryptographic proof that `C` is an ancestor of `F` and not Byzantine-independence proof. The retained `bc7000e` decision and all three historical release policies above do not authorize the recovery source. A release containing this path is eligible only after it is committed and pushed, audited by newly designated agents, instantiated as an exact generation-2 release policy, and separately confirmed by the owner through the v5 TTY ceremony. This document records no generation-2 owner confirmation, signature, send, receipt, pool, liquidity, or LP position.
