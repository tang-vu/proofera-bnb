...
The current recovery design separates a `300`-second coordinator envelope, at most `240` seconds for owner entry, an exact `60`-second execution-authority lifetime, and a `30`-second post-claim freshness limit. Before slot-1 claim it additionally requires at least `50` seconds remaining and no more than `10` seconds since confirmation; a successful post-claim recheck must leave `20` seconds for the remaining execution path. Recovery generation 4 binds the exact generation-3 fence and attempt through owner-v7 domains and uses policy TTY challenge/frame v5. Only an exact owner-byte match permits the internal clock to capture `confirmedAt`; persisted bytes, chat, clipboard content, and predecessor records do not recreate authority.
...
Recovery generation 4 is deliberately incident-specific. Invocation A requires the immutable generation-1 and generation-2 fences plus only the exact expired generation-3 claim, exact-empty active generation-4 and old/new submission namespaces, and a fresh read-only no-effect snapshot. It rereads all six durable namespaces, appends/rereads the generation-3 slot-2 fence with `O_EXCL`/fsync/self-hash, and must stop with `PREDECESSOR_FENCE_RECORDED_RESTART_REQUIRED`. Only a separate invocation B may admit a newly audited policy through TTY v5, obtain snapshot B strictly after the fence, and request owner-v7 confirmation. Active signing uses `bsc-testnet-pta-wbnb-pool-v4`; predecessor submission-v2 is read-only, while new submission state uses `bsc-testnet-pta-wbnb-pool-submission-v3` with v5 journal records.

A separate submission/reconciliation stack validates the one exact generation-4 signed transaction, fresh dual-RPC pre-submission evidence, the recovery quartet, a v5 durable owner-authorization/submission journal, restart-first recovery, winner-only durable `submission_started`, and a second terminal dual-RPC reread immediately before the single private send attempt. Known post-claim failures before worker authorization compete for slot 2 as durable `failed_before_worker`; ambiguous crashes remain claim-only and non-retryable. Terminal confirmation still requires the provider-attested receipt-plus-128 sandwich and receipt-block EIP-1898 post-state/logs; this is bounded dual-provider RPC consistency, not cryptographic or Byzantine-independence proof. Eligibility requires a newly committed/pushed generation-4 release, two new exact audits, a new canonical policy, invocation-A fence/restart, and separate owner-v7 confirmation. This document records no generation-4 owner confirmation, signature, send, receipt, pool, liquidity, or LP position.
