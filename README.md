...
TTY policy transport v2 is a nonce-bound, bounded ASCII `BEGIN`/`CHUNK`/`END` state machine. Every line binds the challenge nonce, exact line index, chunk count, policy byte length, and lowercase raw policy-byte SHA-256; each `CHUNK` also binds its exact chunk index. Lines must arrive in exact order with no duplicate, gap, blank/control/trailing data, using only LF or CRLF. The reader retains a `4,096`-byte hard line cap, while valid worst-case construction has an exact `2,618`-byte maximum content line and a conservative tested bound of `2,700` bytes. The whole transport is at most `102,400` bytes and the policy is `1..65,536` bytes. Every non-final chunk carries exactly `2,304` base64url characters, `N` is derived from the canonical unpadded-base64url length, and `N` cannot exceed `38`. The historical `11,560`-character policy would occupy six chunks and eight total v2 lines; that arithmetic does not reauthorize it. A one-off local Windows Console observation motivated these conservative constants, but its harness and result are not retained and it is not reproducible repository evidence. Repository unit tests establish only the transport arithmetic and parser behavior. One absolute five-minute deadline covers the entire frame. The decoder reconstructs the bytes once, rechecks canonical base64url, declared length, and SHA-256, and rejects v1, truncation, reorder, missing/duplicate chunks, bad terminators, or buffered trailing input before policy admission, fresh RPC, custody access, or owner authority. These checks do not prove that the OS input queue had no earlier data and do not provide preload or malicious-same-user protection.

A later exact v2 exercise used release commit `36f6e5e7fa8b4b5ccf255a6210afa2d25c25afa5`, tree `54ed5c2a8f754e79080528a2ce25669a6532a66b`, runtime-manifest SHA-256 `0x25e5aedb6d73e6fff416803ce9d42737d9124e525e0b81bf906321f4d06258d4`, and canonical policy digest `0xd1a33479a607d744a51ff8d6d3df8772f41ec2f1363911d2655f926c754c3b38`. The strict TTY-v2 reader admitted that policy. Recovery probes found both signing and submission journals absent; a fresh read-only preflight against the two fixed official RPCs succeeded and the exact owner challenge was displayed. No exact owner confirmation was accepted. Subsequent attempts stopped fail-closed with `POLICY_FRAME_INVALID`, `CEREMONY_IO_FAILED`, and `OWNER_CONFIRMATION_INVALID`. Across those exercises there was no custody-artifact access, DPAPI operation, custody unlock, signature, new transaction hash, send or broadcast, receipt, pool, or liquidity. This is a one-off local terminal observation: its raw output and harness were not retained, so it is not reproducible repository evidence or a release gate. The negative custody/sign/send claims reflect the observed fail-closed result plus code ordering, not a retained artifact. The timing source then changed, so this exact release triplet and policy are historical, non-authorizing inputs for the current source.

The revised runtime separates four timing bounds. The unsigned coordinator envelope lasts exactly `300` seconds. Owner entry is allowed for at most `240` seconds, with the strict deadline `min(challengeIssuedAt + 240 seconds, envelopeExpiresAt - 45 seconds)` so the entire `45`-second execution-authority lifetime remains after confirmation. The v4 owner text and exact-byte-confirmation frame bind `challengeIssuedAt`, `confirmationNotAfter`, the exact 45-second lifetime, and the deterministic derivation rule; they cannot bind future clock values. Only after an exact byte match does the internal clock capture actual `confirmedAt`, and the `WeakMap`-branded v4 command binds that value and `executionExpiresAt = confirmedAt + 45 seconds`. A post-claim dual-RPC capability may refresh its observation/authentication timestamp to a value at or after `confirmedAt`, must keep that same execution expiry, and is usable only while the recheck is at most `30` seconds old. These clocks are separate from the unchanged five-minute TTY-v2 policy-frame deadline and its existing transport limits.

A separate submission/reconciliation stack validates the one exact signed transaction, fresh dual-RPC pre-submission evidence, a v2 durable owner-authorization/submission journal, restart-first recovery, winner-only durable `submission_started`, and a second terminal dual-RPC reread after that durable acknowledgement and immediately before the single private send attempt. Ambiguous or restarted outcomes reconcile without resend or replacement. For each fixed provider, terminal confirmation uses the provider-attested fixed receipt-plus-128 sandwich `F1 -> C1/ancestry -> C2 -> F2 -> EIP-1898(C)`, plus receipt-block EIP-1898 post-state/logs. The finalized heads may differ, but both providers must agree on the checkpoint, ancestry, canonical attestation, and receipt-block post-state/logs. This is bounded dual-provider RPC consistency evidence, not a cryptographic proof that `C` is an ancestor of `F` and not Byzantine-independence proof. The retained `bc7000e` decision, historical v1 triplet/policy, and now-stale `36f6e5e7` v2 triplet/policy do not authorize the current timing-changed source. A release containing this path is eligible only after it is committed and pushed, audited by newly designated agents, instantiated as an exact release policy, and separately confirmed by the owner on the TTY. This document records no exact owner transaction approval, signature, send, receipt, pool, liquidity, or LP position.
