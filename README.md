...
An isolated fallback token exists at `contracts/testnet-fixed-asset`: ProofEra Test Asset (`PTA`) is a chain-97-only, fixed-supply, non-economic ERC-20 with no admin or later-mint surface. Its compiler/build/ABI/bytecode preparation is reproducibly tested. A separate deterministic command prepares only a zero-value PTA/WBNB review call tuple, with no complete or serialized transaction request, signed or unsigned envelope, or guessed pool address and twelve blockers left open. PTA remains undeployed, unpriced and illiquid; neither preparation makes a pool or activation path eligible. Deploying it, creating or funding a pool, or using paid explorer verification requires explicit approval.

The dedicated PTA deployer now also has a server-only, read-only deployment-observation boundary. Two official BNB testnet RPC origins agreed on chain `97`, one finalized block, the EOA/nonce, empty predicted CREATE target, legacy gas price, gas estimate, and exact simulated PTA runtime. A later finalized funding transfer supplied `0.1 tBNB`; both official providers still report deployer nonce `0`, empty deployer/target code and target nonce `0`. The exact one-shot deployment is hard-pinned to unused nonce `0` and predicted contract `0x4ed64525d6fB06b7dA926C683CBD809632C9B4Cc`; a used nonce blocks instead of silently changing the target. A pure serializer can revalidate a fresh funded envelope and produce the canonical nine-field EIP-155 signing payload, but it is explicitly non-broadcastable and non-authorizing. No live unsigned payload, signer, signature, signed deployment transaction, broadcast, deployment, or deployment receipt exists.
...
| `contracts/testnet-fixed-asset` | Isolated, undeployed fixed-supply BSC-testnet fixture plus unsigned deployment and pool-initializer preparation |
