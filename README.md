...
The runtime separates four timing bounds: a `300`-second coordinator envelope, at most `240` seconds for owner entry, an exact `45`-second execution-authority lifetime, and a `30`-second post-claim freshness limit. The owner deadline remains `min(challengeIssuedAt + 240 seconds, envelopeExpiresAt - 45 seconds)`. Recovery generation 3 preserves those limits, binds the exact generation-2 fence and attempt through owner-v6 domains, and uses policy TTY challenge/frame v4. Only an exact owner-byte match permits the internal clock to capture `confirmedAt`; persisted bytes, chat, clipboard content, and predecessor records do not recreate authority.
...
Recovery generation 3 is deliberately incident-specific. It requires the immutable generation-1 fence plus only that exact expired generation-2 claim, with no later generation-2 slot and exact-empty generation-3 signing/submission state. Invocation A obtains a fresh read-only no-effect snapshot, rereads all four durable namespaces, appends the generation-2 slot-2 fence with `O_EXCL`/fsync/self-hash while separately retaining the ancestor fence hash, rereads it, and must stop with `PREDECESSOR_FENCE_RECORDED_RESTART_REQUIRED`. Only a separate invocation B may admit a newly audited policy through TTY v4, obtain snapshot B strictly after the fence, and request owner-v6 confirmation. Active signing uses `bsc-testnet-pta-wbnb-pool-v3`; the visible submission directory remains `bsc-testnet-pta-wbnb-pool-submission-v2`, but current schema and filenames are v4.

A separate submission/reconciliation stack validates the one exact generation-3 signed transaction, fresh dual-RPC pre-submission evidence, the recovery quartet, a v4 durable owner-authorization/submission journal, restart-first recovery, winner-only durable `submission_started`, and a second terminal dual-RPC reread immediately before the single private send attempt. Ambiguous or restarted outcomes reconcile without resend or replacement. Terminal confirmation still requires the provider-attested receipt-plus-128 sandwich and receipt-block EIP-1898 post-state/logs; this is bounded dual-provider RPC consistency, not cryptographic or Byzantine-independence proof. Historical policies do not authorize generation 3. Eligibility requires a newly committed/pushed release, new exact audits/policy, and separate owner-v6 TTY confirmation. This document records no generation-3 owner confirmation, signature, send, receipt, pool, liquidity, or LP position.
