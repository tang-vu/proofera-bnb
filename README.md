...
A local post-claim recheck core requires an already authenticated exact intent before it can compare the two fixed official RPCs, bind common-finalized EIP-1898 state, repeat nonce/pool/candidate/simulation/balance/gas checks, and issue a short-lived in-memory capability. The production path now includes a same-process two-phase native-TTY owner ceremony with a 32-byte OS-CSPRNG nonce and exact digest-bound confirmation, plus a closure-private native worker bridge whose execution capability can be reserved once and is consumed only after the signing journal durably starts the worker. The root command is data- and capability-gated: it first performs read-only recovery, then requires a release-review policy bound to its exact clean published commit/tree/runtime manifest and finally the owner's exact TTY confirmation. The generic raw sender and public worker factory remain hard-blocked; only the private, journal-bound one-send bridge is reachable after all gates succeed.

A separate submission/reconciliation stack validates the one exact signed transaction, fresh dual-RPC pre-submission evidence, a v2 durable owner-authorization/submission journal, restart-first recovery, winner-only durable `submission_started`, and a second terminal dual-RPC reread after that durable acknowledgement and immediately before the single private send attempt. Ambiguous or restarted outcomes reconcile without resend or replacement; terminal validation requires matching dual-provider transaction/receipt/logs, EIP-1898 post-state at the receipt block, and exact ancestry from that block to the fixed finalized checkpoint 128 blocks later. The retained `bc7000e` decision does not cover this changed release. Until this release is committed, pushed, reviewed by the owner-designated agents and instantiated as an exact runtime policy, the command cannot pass its policy gate. No exact owner transaction approval, signature, send, receipt, pool, or LP position exists yet.
