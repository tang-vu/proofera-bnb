...
The current recovery design separates a `300`-second coordinator envelope, at most `240` seconds for owner entry, an exact `120`-second execution-authority lifetime, and a `30`-second post-claim freshness limit. Before slot-1 claim it additionally requires at least `60` seconds remaining and no more than `60` seconds since confirmation; a successful post-claim recheck must leave `20` seconds for the remaining execution path. Recovery generation 5 directly binds the exact generation-4 terminal and attempt through owner-v8 domains, uses policy TTY challenge/frame v6, and pins the only permitted initializer transaction to sender nonce `9`. Non-pool chain-97 work advanced the shared test EOA after generation 4; generation 5 had not signed or submitted, so its unused current namespace was rebased while immutable generation-1..4 evidence remains nonce-`1` history. Any further nonce change blocks before signing. Only an exact owner-byte match permits the internal clock to capture `confirmedAt`; persisted bytes, chat, clipboard content, and predecessor records do not recreate authority.
