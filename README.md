...
A local post-claim recheck core requires an already authenticated exact intent before it can compare the two fixed official RPCs, bind common-finalized EIP-1898 state, repeat nonce/pool/candidate/simulation/balance/gas checks, and issue a short-lived in-memory capability. The non-executing production hardening now also includes a same-process two-phase native-TTY owner ceremony with a 32-byte OS-CSPRNG nonce and exact digest-bound confirmation, plus a closure-private native worker bridge whose execution capability can be reserved once and is consumed only after the signing journal durably starts the worker. These controls are local implementation evidence only: the root production runner and raw broadcaster still hard-block with `PRODUCTION_AUTHORIZATION_UNAVAILABLE`, so no executable sign/send path exists.

A separate submission/reconciliation stack locally implements and validates the one exact signed transaction, fresh dual-RPC pre-submission evidence, a v2 durable owner-authorization/submission journal, restart-first recovery, winner-only durable `submission_started`, and a second terminal dual-RPC reread after that durable acknowledgement and immediately before the single send attempt. Ambiguous or restarted outcomes reconcile without resend or replacement; terminal validation requires matching dual-provider transaction/receipt/log/EIP-1898 post-state and an exact receipt-to-common-finalized ancestry bounded to 128 blocks. The current changed code release has no owner-designated final decision bound to its exact commit; the retained `bc7000e` decision does not cover it. The root runner and raw broadcaster remain hard-blocked, and there is no exact owner transaction approval, signature, send, receipt, pool, or LP position.
