...
PTA remains unpriced and illiquid. Both fixed chain-97 RPCs now observe the exact generation-10 status-1 initializer transaction `0xa24d1dfa3440de3fcb644d9b52847bcc8d54f43a2e29b425f50bbce4bd684022`, the expected `PoolCreated`/`Initialize` logs, and pool address `0x30b07e82d7181a53Ae2EA98Cd08b6733Ffd831aE`. Historical EIP-1898 checkpoint and receipt-block state are unavailable from those providers, so strict durable reconciliation remains `do_not_retry/CANONICALITY_INVALID` and ProofEra makes no terminal `confirmed` claim. The pool has no observed liquidity or LP position. The provisional LP envelope of at most `1,000 PTA` plus `0.001 WBNB` remains unapproved and has no funding, wrapping, allowance, mint calldata, or receipt behind it. Funding liquidity, minting an LP position, protocol execution beyond the consumed initializer authorization, or paid explorer verification remains separately gated.
...
- PLACEHOLDER_MISSION — verified historical grant/execute/revoke receipts, final authority absence, and the no-active-session state;
