...
[**Live marketplace**](https://proofera.tangvu.dev) · [**Watch the 5:25 demo**](https://youtu.be/ron927GeVXI) · [**Proof Room**](https://proofera.tangvu.dev/proof) · [**Agent Advantage Report**](./docs/agent-advantage-report.md)
...
Current judge-facing build `dcd1de0789b8d6180dd5445f4bb3ef7afcc276f8` passed clean-checkout CI, a retained two-resolver/five-host TLS/eleven-response rehearsal, and GitHub-hosted exact-build uptime run [`34093342689`](https://github.com/tang-vu/proofera-bnb/actions/runs/34093342689). The checks bind exact build identity, submitted-entry visibility, and the intentionally disabled capital boundary. Product ancestor `f25a67daa0292b5a04c142a42606888f7ec2b8e6` retains the richer four-category `land → find → inspect current evidence → activate analysis → inspect run` journey: 20 source-specific facts, eight screenshots, four completed A2A runs and four hashed responses. Readiness v2 reports analysis-service activation as implemented while keeping capital activation and judging readiness false. Current-source evidence remains separate from analyzer presets; this is not performance, economic-benefit or transaction evidence.
...
PTA remains a non-economic, unpriced test asset; the arbitrary seed ratio is not a market price, peg, oracle or valuation. Both fixed chain-97 RPCs observe the exact generation-10 status-1 initializer transaction `0xa24d1dfa3440de3fcb644d9b52847bcc8d54f43a2e29b425f50bbce4bd684022`, expected `PoolCreated`/`Initialize` logs and pool `0x30b07e82d7181a53Ae2EA98Cd08b6733Ffd831aE`; the initializer's historical EIP-1898 limitation remains unchanged. A separately authorized first-LP run on release `2f7eb8c41ae01843ae47e8a182241d055ca4d1ab` then sent exact approval transaction `0x001c0e6c2f4fc567a455bcb0cd44be6c9ba768066551bdf1d94d978a33138c9d` and direct mint transaction `0xeed9c32a107b57735f45bd6246d967cb12fbb1579a05faa9f17e0ead46187d01` at most once each. Both fixed RPCs agree on their successful finalized receipts and EIP-1898 mint-block post-state: full-range fee-500 NFT `37109` belongs to the owner, exact raw liquidity is `10^18`, consumption is `1,000 PTA + 0.001` native BNB, and residual manager allowance is zero. The retained [execution artifact](./evidence/onchain/bsc-testnet-pta-wbnb-first-lp-eed9c32a107b57735f45bd6246d967cb12fbb1579a05faa9f17e0ead46187d01.json) has SHA-256 `3fa80573ea8cd3ee85208670048bffed48d757c2e8674757ac3331077f121d6a`. A later [bounded outcome](./evidence/pancake/runs/pta-wbnb-outcomes/127841040-127861271.json) over `9,104` seconds found unchanged price/liquidity, zero recomputed fees and zero IL only within that unchanged window, while retaining `64,146,300,000,000` testnet wei of gas cost. Its result is no observed benefit. Because the mint was owner-executed and the frozen manual task is unrelated, neither artifact proves autonomous-agent advantage, market demand, profit or realized economic benefit.
...
Recovery generation 10 is now append-only reconciliation state. It independently validated and recovered only the exact generation-9 signature, admitted policy/runtime v10 through TTY challenge/frame v11, accepted separate owner-v13 confirmation, and authorized zero additional signatures plus at most one send. The host journal durably contains the exclusive generation-10 `submission_started` record, so every restart bypasses policy/owner/send and is reconciliation-only with no resend or replacement. Both fixed RPCs return matching normalized transaction, status-1 receipt, expected `PoolCreated`/`Initialize` payloads, receipt block and receipt-plus-128 ancestry for `0xa24d1dfa3440de3fcb644d9b52847bcc8d54f43a2e29b425f50bbce4bd684022`. Both lack the required historical EIP-1898 checkpoint attestation and receipt-block post-state, so the strict runner returns `do_not_retry/CANONICALITY_INVALID` and creates no durable terminal claim. The retained bounded observation is explorer-verifiable receipt evidence, not authenticated raw-RPC provenance or ProofEra strict current pool-state proof. That initializer run itself added no liquidity or LP position and granted no authority for the later separately confirmed mint.
...
| ---------------------- | ----------: | ------------------------------------------------------------------------------- |
| LP Range               |          17 | public analyzer + ID 1825 + two hires; no execution/performance claim           |
| Grid Trading           |          24 | public analyzer + ID 1826; no paid hire, market fetch, or execution claim       |
| Yield Optimisation     |          33 | public analyzer + ID 1827; no source-attested route or execution claim          |
| Health-Factor Guardian |          42 | public analyzer + ID 1828 + one hire; publication/activation flags remain false |
...
The SHA-pinned CI workflow has root verification, a digest-pinned PostgreSQL 17.9 job for both activation ledgers, four fail-independent reference-agent matrix jobs, isolated contract jobs, and isolated Playwright. Hosted [Actions run 34092851888](https://github.com/tang-vu/proofera-bnb/actions/runs/34092851888) passed all nine jobs from clean checkout at deployed commit `dcd1de0789b8d6180dd5445f4bb3ef7afcc276f8`, including the full verification/audit/license path and 118 passing E2E tests with two intentional live opt-in skips. Hosted [public uptime run 34093342689](https://github.com/tang-vu/proofera-bnb/actions/runs/34093342689) passed all eleven surfaces for that exact deployed build; the scheduled workflow reads the validated deployed-build repository variable and repeats the read-only check twice per hour through judging. The marketplace and four read-only analyzer endpoints are public and exact-build monitored. Retained ProofEra testnet writes now include PTA deployment, four chain-97 ERC-8004 registrations, three paid-hire receipts, the bounded Altana grant/execute/revoke receipts, and the exact two-transaction controlled Pancake first-LP run. The Altana execute action is only PTA `approve(session, 0)` and did not authorize the separate LP run. The LP page now explains the one-grant mandate model, Session Control exposes the bounded testnet grant/revoke flow, and Mission Control shows the retained receipt chain plus final authority absence; production autonomous LP policy handoff remains unavailable. `/api/health` is liveness only. `/api/readiness` intentionally remains HTTP `503 not_ready`, but schema v2 separates `readyForAnalysisActivation=true` from `readyForCapitalActivation=false` and `readyForJudging=false`; the capital path still requires the production LP write target, eligible live mandate, deployed database, authenticated signer handoff and exact production authority probe.

Useful local routes:

- `/marketplace` — immediate intent, four public registered analyzer dossiers, finalized identity/hire layers, and independent live ERC-8004 ingress with explicit pending/empty/unavailable/available states;
- `/pancake-position` — user-supplied Pancake V3 position, pool, and atomic one-block evidence boundary;
- `/lp-activate` — testnet-only one-grant LP mandate configuration; trusted autonomous policy handoff and LP authority remain absent;
- `/session-control` — final-origin passkey onboarding plus the bounded Altana testnet grant, automatic in-scope action, and owner revoke controls;
...
- `/mission-control` — verified historical grant/execute/revoke receipts, final authority absence, and the no-active-session state;
...
When the durable local marketplace already owns Playwright's default port `3217`, choose a free
loopback test port without stopping production:

```powershell
$env:PROOFERA_E2E_PORT = "3218"
try { pnpm test:e2e } finally { Remove-Item Env:PROOFERA_E2E_PORT }
```

The test server validates the numeric port and uses an ignored `.tmp/next-e2e-<port>` build directory,
so a development test cannot replace the active production `.next` bytes.
...
- [Hackathon score and deduction audit](./docs/hackathon-score-strategy.md)
- [Dependency and license boundary](./docs/dependencies.md)
...
The generated LP reference workspace is under `agents/lpRangeAgent`. Its public analyzer endpoint and BSC-testnet Agent ID 1825 are verified, and two finalized paid-hire receipts exist. Those layers do not prove task completion, execution authority, or performance.
