...
Recovery generation 8 is the current append-only incident path. It binds the immutable generation-7 terminal and inherited generation-6 terminal, requires submission-v6 to contain no retained transaction state, and requires a fresh distinct envelope after the generation-7 terminal. Startup rereads fourteen isolated namespaces: signing generations 1 through 8 and submission v2 through v7. Policy/runtime-instantiation is v8, TTY challenge/frame is v9, owner command/text/confirmation is v11, signing/authorized intent/broadcast is v8, active signing is `bsc-testnet-pta-wbnb-pool-v8`, and active submission is `bsc-testnet-pta-wbnb-pool-submission-v7` with v9 schema/files and durable owner policy v9. The post-claim RPC reader issues ordered dual-provider pairs with at most one request per endpoint in flight and no retry; known failures retain their exact code while ambiguous transport outcomes remain non-retryable. The timing/cap sequence remains `300/240/120/60/60/20/30` seconds.

The submission/reconciliation stack validates the one exact generation-8 signed transaction, fresh dual-RPC pre-submission evidence, the recovery quartet, restart-first recovery, winner-only durable `submission_started`, and a second terminal dual-RPC reread immediately before the single private send attempt. Known post-claim failures before worker authorization durably record `failed_before_worker`; ambiguous crashes remain non-retryable and reconciliation-only. Terminal confirmation still requires the provider-attested receipt-plus-128 sandwich and receipt-block EIP-1898 post-state/logs; this is bounded dual-provider RPC consistency, not cryptographic or Byzantine-independence proof. Eligibility requires a newly committed/pushed generation-8 release, two new exact audits, a new canonical policy, and separate owner-v11 confirmation. This document records no generation-8 owner confirmation, signature, send, receipt, pool, liquidity, or LP position.
