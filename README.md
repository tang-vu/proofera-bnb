...
Recovery generation 7 is the current append-only incident path. It binds the immutable generation-6 terminal and its inherited generation-5 terminal, requires submission-v5 to contain no retained transaction state, and requires a fresh distinct envelope after the generation-6 terminal. Startup rereads twelve isolated namespaces: signing generations 1 through 7 and submission v2 through v6. Policy/runtime-instantiation is v7, TTY challenge/frame is v8, owner command/text/confirmation is v10, signing/authorized intent/broadcast is v7, active signing is `bsc-testnet-pta-wbnb-pool-v7`, and active submission is `bsc-testnet-pta-wbnb-pool-submission-v6` with v8 schema/files and durable owner policy v8. The exact timing/cap sequence remains `300/240/120/60/60/20/30` seconds.

The submission/reconciliation stack validates the one exact generation-7 signed transaction, fresh dual-RPC pre-submission evidence, the recovery quartet, restart-first recovery, winner-only durable `submission_started`, and a second terminal dual-RPC reread immediately before the single private send attempt. Known post-claim failures before worker authorization durably record `failed_before_worker`; ambiguous crashes remain non-retryable and reconciliation-only. Terminal confirmation still requires the provider-attested receipt-plus-128 sandwich and receipt-block EIP-1898 post-state/logs; this is bounded dual-provider RPC consistency, not cryptographic or Byzantine-independence proof. Eligibility requires a newly committed/pushed generation-7 release, two new exact audits, a new canonical policy, and separate owner-v10 confirmation. This document records no generation-7 owner confirmation, signature, send, receipt, pool, liquidity, or LP position.
