...
The dedicated PTA deployer now also has a server-only, read-only deployment-observation boundary. Two official BNB testnet RPC origins agreed on chain `97`, one finalized block, the unfunded EOA/nonce, empty predicted CREATE target, legacy gas price, gas estimate, and exact simulated PTA runtime. The exact one-shot deployment is hard-pinned to unused nonce `0` and predicted contract `0x4ed64525d6fB06b7dA926C683CBD809632C9B4Cc`; a used nonce blocks instead of silently changing the target. The retained observation is correctly `blocked` because the wallet balance is `0`; its bounded maximum deployment cost was `67,417,100,000,000` wei at capture time. A pure serializer can revalidate a funded envelope and produce the canonical nine-field EIP-155 signing payload, but it is explicitly non-broadcastable and non-authorizing. No live unsigned payload, signer, signature, signed transaction, broadcast, deployment, or receipt exists.
