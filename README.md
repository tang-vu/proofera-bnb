...
Generation 8 is now historical. The owner entered exact owner-v11 bytes for release `08f0357f1281c2289a1a0db9637e8fb082cb6900`. The local journal retained `01-claim.v8.json` (`1,364` bytes, raw SHA-256 `0x5a85737428a4bbd06459ceab52d6096fba74aa1c002de31a24c942ff9f3954f6`) and `02-transition.v8.json` (`1,369` bytes, raw SHA-256 `0x3210fd8ab08c2282a5da1aeb426984592fed9a5b3a6832ac7d60991baaf4fc6d`). Its terminal is `failed_before_worker` / `GAS_POLICY_VIOLATION`, with outcome digest `0x15b8bd2046fdac833c932d21deea39e7901bb97398622ad03e7625167e19d469`. The fresh estimate was `5,012,641`, above the then-authorized `5,000,000` maximum. No worker authorization/start, custody unlock, signature, submission, transaction hash, receipt, pool, liquidity, or LP position exists.

Recovery generation 9 is the current append-only incident path. It binds the immutable generation-8 terminal and inherited generation-7 terminal, requires submission-v7 to contain no retained transaction state, and requires a fresh distinct envelope after the generation-8 terminal. Startup rereads sixteen isolated namespaces: signing generations 1 through 9 and submission v2 through v8. Policy/runtime-instantiation is v9, TTY challenge/frame is v10, owner command/text/confirmation is v12, signing/authorized intent/broadcast is v9, active signing is `bsc-testnet-pta-wbnb-pool-v9`, and active submission is `bsc-testnet-pta-wbnb-pool-submission-v8` with v10 schema/files and durable owner policy v10. The post-claim RPC reader issues ordered dual-provider pairs with at most one request per endpoint in flight and no retry; known failures retain their exact code while ambiguous transport outcomes remain non-retryable. The timing/cap sequence remains `300/240/120/60/60/20/30` seconds.

The submission/reconciliation stack validates the one exact generation-9 signed transaction, fresh dual-RPC pre-submission evidence, the recovery quartet, restart-first recovery, winner-only durable `submission_started`, and a second terminal dual-RPC reread immediately before the single private send attempt. Known post-claim failures before worker authorization durably record `failed_before_worker`; ambiguous crashes remain non-retryable and reconciliation-only. Terminal confirmation still requires the provider-attested receipt-plus-128 sandwich and receipt-block EIP-1898 post-state/logs; this is bounded dual-provider RPC consistency, not cryptographic or Byzantine-independence proof. Eligibility requires a newly committed/pushed generation-9 release, two new exact audits, a new canonical policy, and separate owner-v12 confirmation. This document records no generation-9 owner confirmation, signature, send, receipt, pool, liquidity, or LP position.
