...
- Target activation model: scoped Altana sessions with contract/function allowlists, spend caps, expiry, and verified revoke. Local policy/bootstrap/lifecycle, one-shot SDK grant, and canonical append-only PostgreSQL grant-claim boundaries exist. Both ledgers are deployed on the host-local staging boundary and pass their canonical read-only verifier, but no worker is connected and no wallet grant is represented as complete.
...
The current recovery design separates a `300`-second coordinator envelope, at most `240` seconds for owner entry, an exact `120`-second execution-authority lifetime, and a `30`-second post-claim freshness limit. Before slot-1 claim it additionally requires at least `60` seconds remaining and no more than `60` seconds since confirmation; a successful post-claim recheck must leave `20` seconds for the remaining execution path. Recovery generation 5 directly binds the exact generation-4 terminal and attempt through owner-v8 domains and uses policy TTY challenge/frame v6. Only an exact owner-byte match permits the internal clock to capture `confirmedAt`; persisted bytes, chat, clipboard content, and predecessor records do not recreate authority.
...
Recovery generation 5 is the current incident-specific design. It directly binds that immutable generation-4 terminal raw hash and separately requires submission-v3 to be exact-empty; it does not use an Invocation A, a new no-effect proof, or a reusable fence snapshot. A fresh, distinct execution envelope observed after the terminal is still required. Startup rereads eight isolated namespaces: signing generations 1 through 5 and submission v2 through v4. Current policy/runtime-instantiation is v5, TTY challenge/frame is v6, owner command/text/confirmation is v8, signing/authorized intent/broadcast is v5, and active submission is `bsc-testnet-pta-wbnb-pool-submission-v4` with schema/file generation v6. The exact timing/cap sequence is `300/240/120/60/60/20/30` seconds.

A separate submission/reconciliation stack validates the one exact generation-5 signed transaction, fresh dual-RPC pre-submission evidence, the recovery quartet, a v6 durable owner-authorization/submission journal, restart-first recovery, winner-only durable `submission_started`, and a second terminal dual-RPC reread immediately before the single private send attempt. Known post-claim failures before worker authorization durably record `failed_before_worker`; ambiguous crashes remain claim-only and non-retryable. Terminal confirmation still requires the provider-attested receipt-plus-128 sandwich and receipt-block EIP-1898 post-state/logs; this is bounded dual-provider RPC consistency, not cryptographic or Byzantine-independence proof. Eligibility requires a newly committed/pushed generation-5 release, two new exact audits, a new canonical policy, and separate owner-v8 confirmation. This document records no generation-5 owner confirmation, signature, send, receipt, pool, liquidity, or LP position.

The Altana handoff rebuilds from recursively snapshotted raw intent/server evidence and accepts only a nominal PostgreSQL `consumeOrRead` capability produced after exact schema and application-access verification. Its atomic receipt binds context, quote, user, policy, write-target attestation, and expiry. Policy and attestation must agree on the exact block number, hash, and timestamp; a final monotonic sample rechecks both reservation expiry and target freshness, and a failed final check cannot retain a nested ready bootstrap. The concrete server-only PostgreSQL implementation has a versioned append-only schema, an externally pinned PostgreSQL 17 catalog digest, a nominal administrator-verification capability, a least-privilege application pool, bounded transaction outcomes, and 10 passing real-PostgreSQL 17.9 cases. A host-local PostgreSQL 17.9 instance now passes the administrator catalog verifier and direct-login application probe; it remains disconnected from any worker, and there is still no live worker authority, grant, or transaction.
...
The SHA-pinned CI workflow has root verification, a digest-pinned PostgreSQL 17.9 job for both activation ledgers, four fail-independent reference-agent matrix jobs, an isolated fixed-asset contract job, and an isolated Playwright job. A workflow definition and local passing gates are not a hosted green run. The marketplace and four read-only analyzer endpoints are public and exact-build monitored. PTA deployment and the four verified chain-97 ERC-8004 registrations are the retained ProofEra testnet writes; no ProofEra Altana grant, Pancake operation, revoke, or performance transaction exists. The final-origin passkey onboarding control does not change that claim. The permission-preview model/renderer is not wired into `/lp-activate`; Mission Control remains an honest no-authority surface. `/api/health` is liveness only, while `/api/readiness` intentionally remains `503 not_ready` until the reviewed write target, eligible pool, deployed database, worker signer handoff, exact authority probe, and completed passkey ceremony are all configured and probed.
