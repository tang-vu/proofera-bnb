...
The current recovery design separates a `300`-second coordinator envelope, at most `240` seconds for owner entry, an exact `120`-second execution-authority lifetime, and a `30`-second post-claim freshness limit. Before slot-1 claim it additionally requires at least `60` seconds remaining and no more than `60` seconds since confirmation; a successful post-claim recheck must leave `20` seconds for the remaining execution path. Recovery generation 6 directly binds the exact generation-5 terminal and attempt through owner-v9 domains, uses policy TTY challenge/frame v7, and pins the only permitted initializer transaction to sender nonce `9`. Any nonce drift blocks before signing. Only an exact owner-byte match permits the internal clock to capture `confirmedAt`; persisted bytes, chat, clipboard content, predecessor records, and the expired generation-5 confirmation do not recreate authority.
...
Recovery generation 6 is the current append-only incident path. It directly binds the immutable generation-5 terminal and its inherited generation-4 terminal, and requires submission-v4 to contain no retained transaction state; it has no Invocation A, new no-effect proof, or reusable fence snapshot. A fresh, distinct execution envelope after the generation-5 terminal is mandatory. Startup rereads ten isolated namespaces: signing generations 1 through 6 and submission v2 through v5. Policy/runtime-instantiation is v6, TTY challenge/frame is v7, owner command/text/confirmation is v9, signing/authorized intent/broadcast is v6, active signing is `bsc-testnet-pta-wbnb-pool-v6`, and active submission is `bsc-testnet-pta-wbnb-pool-submission-v5` with v7 schema/files and durable owner policy v7. The exact timing/cap sequence remains `300/240/120/60/60/20/30` seconds.

The submission/reconciliation stack validates the one exact generation-6 signed transaction, fresh dual-RPC pre-submission evidence, the recovery quartet, restart-first recovery, winner-only durable `submission_started`, and a second terminal dual-RPC reread immediately before the single private send attempt. Known post-claim failures before worker authorization durably record `failed_before_worker`; ambiguous crashes remain non-retryable and reconciliation-only. Terminal confirmation still requires the provider-attested receipt-plus-128 sandwich and receipt-block EIP-1898 post-state/logs; this is bounded dual-provider RPC consistency, not cryptographic or Byzantine-independence proof. Eligibility requires a newly committed/pushed generation-6 release, two new exact audits, a new canonical policy, and separate owner-v9 confirmation. This document records no generation-6 owner confirmation, signature, send, receipt, pool, liquidity, or LP position.
