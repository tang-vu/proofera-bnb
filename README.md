...
An isolated fallback token exists at `contracts/testnet-fixed-asset`: ProofEra Test Asset (`PTA`) is a chain-97-only, fixed-supply, non-economic ERC-20 with no admin or later-mint surface. Its compiler/build/ABI/bytecode preparation is reproducibly tested, and the exact artifact is now deployed on BSC Testnet with a finalized two-provider receipt. A separate deterministic command prepares only a zero-value PTA/WBNB review call tuple, with no complete or serialized transaction request, signed or unsigned envelope, or guessed pool address and twelve blockers left open. PTA remains unpriced and illiquid; deployment alone makes no pool or activation path eligible. Creating or funding a pool, protocol execution, or paid explorer verification remains separately gated.
