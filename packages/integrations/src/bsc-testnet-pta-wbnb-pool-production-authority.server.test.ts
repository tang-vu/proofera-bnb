...
const EXECUTION_EXPIRES_AT = "2026-08-13T04:31:55.002Z";
...
function runtimeRecoveryForEnvelopeObservedAt(
  ...
  return deeplyFreeze({
    ...
    predecessorTerminal: {
      ...
      recordedAt: new Date(observed - 1_000).toISOString()
    }
  });
}
...
function runtimeReview(
  ...
  const value = deeplyFreeze({
    schemaVersion: 7,
    kind: "automated_release_policy_recovery_envelope_instantiation_v7",
    operationKey: BSC_TESTNET_PTA_WBNB_POOL_OPERATION_KEY,
...
function commandFor(
  ...
  return deeplyFreeze({
    schemaVersion: 10,
    kind: "execute_exact_bsc_testnet_pta_wbnb_pool_recovery_generation_7_once_v10",
...
describe("PTA/WBNB runtime-policy and exact owner authority", () => {
  it("binds the full transaction, release policy, automated instantiation, and no-LP scope", async () => {
    ...
    expect(challenge.ownerAuthorizationText).toContain(
      "decision=AUTHORIZE_FRESH_RECOVERY_GENERATION_7_POOL_INITIALIZATION_SIGNATURE_AND_SUBMISSION_AFTER_APPEND_ONLY_PREDECESSOR_TERMINAL"
    );
    expect(challenge.ownerAuthorizationText).toContain(
      `inheritedPredecessorTerminalRawSha256=${BSC_TESTNET_PTA_WBNB_POOL_GENERATION_5_TRANSITION_RAW_SHA256}`
    );
    expect(challenge.ownerAuthorizationText).toContain("separate_exact_empty_submission_v5");
    expect(challenge.ownerAuthorizationText).not.toContain("GENERATION_6_POOL_INITIALIZATION");
    expect(challenge.ownerAuthorizationText).not.toContain("submission_v4");
    ...
    expect(challenge.ownerAuthorizationText).toContain(
      "confirmationNotAfter=2026-08-13T04:32:48.000Z"
    );
    expect(challenge.ownerAuthorizationText).toContain("executionAuthorizationLifetimeSeconds=120");
    expect(challenge.ownerAuthorizationText).toContain(
    ...
    expect(challenge.ownerConfirmationText).toContain(
      "decision=CONFIRM_FRESH_GENERATION_7_AUTHORIZATION_AFTER_APPEND_ONLY_PREDECESSOR_TERMINAL_ONE_SIGNATURE_AND_ONE_SUBMISSION_NO_RETRY_NO_REPLACEMENT"
    );
    expect(challenge.ownerConfirmationText).not.toContain("GENERATION_6_AUTHORIZATION");
    expect(
  ...
  it("rejects pre-v10 owner replay and every fixed predecessor recovery mutation", async () => {
    const setup = harness();
    const exactDescriptor = descriptor();
    const branded = runtimeReview();
    ...
    const terminal = baseRecovery.predecessorTerminal;
    const {
    ...
    const mutations: unknown[] = [
      ...
      Object.freeze({
        ...baseRecovery,
        predecessorTerminal: deeplyFreeze({ ...terminal, status: "claimed" })
      }),
      Object.freeze({
        ...baseRecovery,
        predecessorTerminal: deeplyFreeze({ ...terminal, workerStartOutcome: "attempted" })
      }),
      Object.freeze({
        ...baseRecovery,
        predecessorTerminal: deeplyFreeze({ ...terminal, submissionJournalState: "not_empty" })
      }),
      Object.freeze({
        ...baseRecovery,
        predecessorTerminal: deeplyFreeze(fenceWithoutSubmissionJournalState)
      }),
      Object.freeze({
        ...baseRecovery,
        predecessorTerminal: deeplyFreeze({
          ...terminal,
      ...
      Object.freeze({
        ...
        predecessorTerminal: deeplyFreeze({
          ...terminal,
          recordedAt: ENVELOPE_OBSERVED_AT
        })
      })
    ];
  ...
  it("performs one same-process exact challenge/confirmation round with a fresh nonce", async () => {
    ...
    expect(result.command.schemaVersion).toBe(10);
    expect(result.command.kind).toBe(
      "execute_exact_bsc_testnet_pta_wbnb_pool_recovery_generation_7_once_v10"
    );
  ...
  it("reserves the exact execution lifetime, enforces both confirmation deadlines, and rejects clock rollback", async () => {
    ...
    const ownerWindow = await run(EXPIRES_AT, [
      issueAt,
      issueAt + 1,
      Date.parse(EXPIRES_AT) - 120_000 - 1
    ]);
    expect(ownerWindow.observedNotAfter).toBe(Date.parse(EXPIRES_AT) - 120_000);
    expect(ownerWindow.result).toMatchObject({ status: "confirmed" });

    const exactOwnerBoundary = await run(EXPIRES_AT, [
      issueAt,
      issueAt + 1,
      Date.parse(EXPIRES_AT) - 120_000
    ]);
    ...
    const envelopeReserve = await run(reserveExpiry, [issueAt, issueAt + 1, issueAt + 60_000 - 1]);
    expect(envelopeReserve.observedNotAfter).toBe(issueAt + 60_000);
