...
The exact runners schema-parse these envelopes before release-state or network access. The v1 LP envelope remains retained but is not executable because its declaration was superseded after protected release-scope drift. A run exists only after the corresponding create-only output is retained and independently reviewed.
