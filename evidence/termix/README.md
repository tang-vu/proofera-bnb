...
Status: **THREE PROTECTED TASK-LEVEL PAIRS VERIFIED / BOUNDED CLAIM ONLY**. Each task retains its immutable agent lane, manual lane, original unverified observational pair and implementation-adjacent self-review. Protected reviewer packet v4 and the owner-designated internal review record materialized separate verified derivatives plus pair-bound adjudications; the compiler-accepted final bundle lives under `evidence/submission/final/termix/`. The results do not establish a universal agent advantage or financial guarantee.

`preregistrations/` contains exactly three schema-validated ProofEra-versus-manual protocols:

| File                            | Task                             | Definition SHA-256                                                 | Current protected state |
| ------------------------------- | -------------------------------- | ------------------------------------------------------------------ | ----------------------- |
| `task-01-lp-range-v2.json`      | PancakeSwap V3 LP range decision | `9ac77645f2dd0ade20203b911cba18ce52b7b016fae8d9e73aa2919440b572ab` | verified derivative     |
| `task-02-permission-audit.json` | Altana/Pancake permission audit  | `1191c85c4f36881be0736ced51fc6c23e24286101543bf0838346b0e2ed95645` | verified derivative     |
| `task-03-venus-health.json`     | Venus health-factor replay       | `c15ed1089fdeb75eab7db3134f08c011fd71bfe02ec2c3dbf3052592973c8c55` | verified derivative     |
...
All three declarations, registered identities, hire receipts, run-order seeds, exact inputs and timed invocations were bound before their retained raw captures. LP uses `pancake-lp-agent-20260818-v4` and `pancake-lp-manual-20260818-v1`; Task 02 uses `permission-audit-agent-20260822-v1` and `permission-audit-manual-20260822-v1`; Venus uses `venus-health-agent-20260818-v2` and `venus-health-manual-20260818-v2`. These source files preserve timing, exact outputs, receipts and limitations but do not self-prove the no-agent boundary, so the original observational pairs correctly remain unverified. The protected derivatives under `final-pairs/` preserve those measurements and add only reviewer-bound verification, QA, evidence state and stale limitations. Task 02 binds the retained Altana lifecycle, code observation, staging database receipt, public proof-worker state, blind corpus, Agent `1825`, verified hire and reviewer-held answer-key digest in [`38046f87b87c-126555555.json`](./declarations/permission-audit/38046f87b87c-126555555.json). Finalized block `126555555` forced manual-first; both exact invocations and both raw captures are retained.

Task 02's deterministic `audit_altana_permission_bundle` v1.1 skill is live on the public LP A2A endpoint and its Agent Card advertises that exact ID. The timed manual and agent lanes both returned the same 18 evidence-linked findings, corrected enforcement table, exact bundle digest and `executionPerformed: false`. The original pair [`permission-audit-pair-20260822-v1.json`](./pairs/permission-audit/permission-audit-pair-20260822-v1.json) records exact monotonic durations (`12,756,400 ns` manual and `2,318,655,500 ns` agent), explicit zero incremental tBNB fees and rubric parity at `100/100`; its companion self-review remains explicitly non-independent. The separate protected derivative and adjudication verify this bounded result without relabelling the source pair.

Task 01's retained pair [`pancake-lp-pair-20260822-v1.json`](./pairs/pancake-lp/pancake-lp-pair-20260822-v1.json), logical pair SHA-256 `41f0df2b0ad2886573cf9e1035b247f971a2fed493b083af87b3153fa6adc103`, recomputes the exact-hash tick/range worksheet and preserves unavailable economics. It records `1,073,806,100 ns` agent versus `620,556,240,100 ns` manual, zero incremental native fees and rubric parity at `100/100`. Task 03's retained pair [`venus-health-pair-20260822-v1.json`](./pairs/venus-health/venus-health-pair-20260822-v1.json), logical pair SHA-256 `2f3ac750834b93802b42f3317d35cfd82cb4381aec2497fb9b88232d6adca600`, recomputes all three integer 18-decimal health factors, their 144-second window and the bounded `hold` decision. It records `856,509,100 ns` agent versus `10,048,900 ns` manual, zero incremental native fees and rubric parity at `100/100`. These source pairs are not publishable by themselves because both self-reviews explicitly set `secondReviewerIndependent: false`; only their separately retained protected derivatives support the bounded final comparison.

The current review contract is [`reviewer-packets/20260826-v4/manifest.json`](./reviewer-packets/20260826-v4/manifest.json), SHA-256 `ace9453b52cd8735fe8bcbeca8f9ed22170852dc50f245ff4aaad60d0f9c97ac`. It binds three source pairs, 27 exact evidence items, payload/final-LF digests and eight protected runtime files. The review record [`reviews/independent/20260826-v4.json`](./reviews/independent/20260826-v4.json), SHA-256 `827005010f8e8f58f77456effe47a78681651671ef4b6eb06553a5a2307a46ce`, records zero P0/P1/P2 findings. Three verified derivatives and three adjudications are retained under `final-pairs/` and `adjudications/`; the final bundle is under [`../submission/final/termix/6d1adf3c948e49be7d9d42332df04904fdd43e3a/`](../submission/final/termix/6d1adf3c948e49be7d9d42332df04904fdd43e3a/).

This is owner-designated internal review, not external or cryptographically identified review. It has no Sigstore attestation, does not prove future runtime/archive availability, did not reinvoke historical A2A calls, and is not transaction authorization.
...
The tests perform no network request, wallet operation, transaction or new timing measurement. They validate the retained result generation and prove production CLIs fail before network/output access when exact invocation material is absent. Task-specific prerequisite and retained invocation material is recorded inside the evidence files.
...
capture beneath `runs/venus-health/`. The valid retained agent capture is
`venus-health-agent-20260818-v2.json`; running the create-only command again
would require a new preregistered run ID rather than overwriting that evidence.
