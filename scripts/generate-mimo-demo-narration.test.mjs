...
test("retained MiMo narration binds exact generated bytes, ASR and decoded media", async () => {
  ...
  assert.equal(manifest.output.probe.durationSeconds, "325.014");
  ...
  const decodedDurationSeconds = Number.parseFloat(media.format.duration);
  const retainedDurationSeconds = Number.parseFloat(manifest.output.probe.durationSeconds);
  const twoMp3FramesSeconds = (2 * 1_152) / 48_000;
  assert.ok(Number.isFinite(decodedDurationSeconds));
  assert.ok(
    Math.abs(decodedDurationSeconds - retainedDurationSeconds) <= twoMp3FramesSeconds,
    `ffprobe duration ${decodedDurationSeconds} drifted by more than two MP3 frames from retained duration ${retainedDurationSeconds}`
  );
