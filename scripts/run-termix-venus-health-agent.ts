...
import { access, lstat, open, readFile, realpath, link, unlink } from "node:fs/promises";
...
const OUTPUT_DIRECTORY = "evidence/termix/runs/venus-health";
...
function validateRepositoryPath(value: string): string {
  if (
    ...
    throw new Error("TERMIX_VENUS_REQUEST_INPUT_PATH_INVALID");
  }
  return value;
}
...
async function readBoundedStdin(): Promise<string> {
  ...
  for await (const chunk of process.stdin) {
    ...
    bytes += buffer.length;
    if (bytes > MAXIMUM_STDIN_BYTES) throw new Error("TERMIX_VENUS_STDIN_TOO_LARGE");
    chunks.push(buffer);
  }
  if (bytes === 0) throw new Error("TERMIX_VENUS_STDIN_REQUIRED");
...
function parseInvocation(text: string): Invocation {
  ...
  try {
    ...
    throw new Error("TERMIX_VENUS_INVOCATION_JSON_INVALID");
  }
  if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) {
    throw new Error("TERMIX_VENUS_INVOCATION_INVALID");
  }
  const record = parsed as Record<string, unknown>;
  if (Object.keys(record).sort().join(",") !== "requestInputSha256,timedRunRequest") {
    throw new Error("TERMIX_VENUS_INVOCATION_INVALID");
  }
  if (
    typeof record.requestInputSha256 !== "string" ||
    !/^[0-9a-f]{64}$/.test(record.requestInputSha256)
  ) {
    throw new Error("TERMIX_VENUS_INVOCATION_DIGEST_INVALID");
  }
  return {
    timedRunRequest: record.timedRunRequest,
    requestInputSha256: record.requestInputSha256
  };
}
...
async function verifyCommittedInput(repositoryPath: string): Promise<string> {
  ...
  if (!workingBytes.equals(committedBytes)) {
    throw new Error("TERMIX_VENUS_REQUEST_INPUT_NOT_COMMITTED");
  }
  const text = workingBytes.toString("utf8");
  if (!text.endsWith("\n") || text.endsWith("\r\n") || text.slice(0, -1).includes("\n")) {
    throw new Error("TERMIX_VENUS_REQUEST_INPUT_FILE_INVALID");
  }
...
async function assertCanonicalPathWithinRepository(absolutePath: string): Promise<void> {
  ...
  if (
    ...
    throw new Error("TERMIX_VENUS_REQUEST_INPUT_PATH_UNTRUSTED");
  }
...
async function assertOutputAvailable(runId: string): Promise<string> {
  ...
  if (local === "" || local.startsWith(`..${sep}`) || isAbsolute(local)) {
    throw new Error("TERMIX_VENUS_OUTPUT_PATH_INVALID");
  }
  try {
    await access(outputPath, constants.F_OK);
    throw new Error("TERMIX_VENUS_OUTPUT_ALREADY_EXISTS");
  } catch (error) {
    if (error instanceof Error && error.message === "TERMIX_VENUS_OUTPUT_ALREADY_EXISTS") {
      throw error;
    }
...
async function main(): Promise<void> {
  const requestInputPath = parseArguments(process.argv.slice(2));
  ...
  const requestInputCanonicalJson = await verifyCommittedInput(requestInputPath);
  if (sha256Bytes(requestInputCanonicalJson) !== invocation.requestInputSha256) {
    throw new Error("TERMIX_VENUS_REQUEST_INPUT_DIGEST_MISMATCH");
  }
  const outputPath = await assertOutputAvailable(timedRequest.runId);
  const capture = await runVenusHealthAgentTermixMethod({
    request: timedRequest,
    requestInputCanonicalJson,
    requestInputSha256: invocation.requestInputSha256,
    clock: {
  ...
  const message =
    error instanceof Error && /^[A-Z0-9_]+$/.test(error.message)
      ...
      : error instanceof Error
        ...
        : "Error";
  process.stderr.write(`TermiX Venus agent runner failed: ${message}\n`);
  process.exitCode = 1;
});
