...
const OUTPUT_DIRECTORY = "evidence/termix/runs/permission-audit/agent";
...
function parseArguments(args: readonly string[]): string {
  ...
  if (
    normalized.length !== 3 ||
    normalized[0] !== "--execute-exact-permission-audit-agent-run" ||
    normalized[1] !== "--input-bundle" ||
    normalized[2] === undefined
  ) {
    throw new Error("TERMIX_PERMISSION_AUDIT_AGENT_CLI_ARGUMENTS_INVALID");
  }
...
function validateRepositoryPath(value: string): string {
  if (
    ...
    throw new Error("TERMIX_PERMISSION_AUDIT_AGENT_INPUT_PATH_INVALID");
  }
  return value;
}
...
async function readBoundedStdin(): Promise<string> {
  ...
  for await (const chunk of process.stdin) {
    ...
    if (bytes > MAXIMUM_STDIN_BYTES) {
      throw new Error("TERMIX_PERMISSION_AUDIT_AGENT_STDIN_TOO_LARGE");
    }
    chunks.push(buffer);
  }
  if (bytes === 0) throw new Error("TERMIX_PERMISSION_AUDIT_AGENT_STDIN_REQUIRED");
...
function parseInvocation(text: string): Invocation {
  ...
  try {
    ...
    throw new Error("TERMIX_PERMISSION_AUDIT_AGENT_INVOCATION_JSON_INVALID");
  }
  if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) {
    throw new Error("TERMIX_PERMISSION_AUDIT_AGENT_INVOCATION_INVALID");
  }
  ...
  if (typeof record.bundleSha256 !== "string" || !/^[0-9a-f]{64}$/u.test(record.bundleSha256)) {
    throw new Error("TERMIX_PERMISSION_AUDIT_AGENT_INVOCATION_DIGEST_INVALID");
  }
...
function verifyReleaseState(sourceCommitSha: string): void {
  if (gitText(["status", "--porcelain=v1", "--untracked-files=all"]) !== "") {
    throw new Error("TERMIX_PERMISSION_AUDIT_AGENT_REPOSITORY_DIRTY");
  }
  const head = gitText(["rev-parse", "HEAD"]);
  const published = gitText(["rev-parse", "origin/main"]);
  if (head !== sourceCommitSha) {
    throw new Error("TERMIX_PERMISSION_AUDIT_AGENT_SOURCE_COMMIT_MISMATCH");
  }
  if (published !== head) throw new Error("TERMIX_PERMISSION_AUDIT_AGENT_SOURCE_NOT_PUBLISHED");
}
...
async function verifyCommittedInput(repositoryPath: string): Promise<string> {
  ...
  if (!workingBytes.equals(committedBytes)) {
    throw new Error("TERMIX_PERMISSION_AUDIT_AGENT_INPUT_NOT_COMMITTED");
  }
  const text = workingBytes.toString("utf8");
  if (!text.endsWith("\n") || text.endsWith("\r\n") || text.slice(0, -1).includes("\n")) {
    throw new Error("TERMIX_PERMISSION_AUDIT_AGENT_INPUT_FILE_INVALID");
  }
...
async function assertCanonicalPathWithinRepository(absolutePath: string): Promise<void> {
  ...
  if (
    ...
    throw new Error("TERMIX_PERMISSION_AUDIT_AGENT_INPUT_PATH_UNTRUSTED");
  }
...
async function assertOutputAvailable(runId: string): Promise<string> {
  ...
  if (local === "" || local.startsWith(`..${sep}`) || isAbsolute(local)) {
    throw new Error("TERMIX_PERMISSION_AUDIT_AGENT_OUTPUT_PATH_INVALID");
  }
  try {
    await access(outputPath, constants.F_OK);
    throw new Error("TERMIX_PERMISSION_AUDIT_AGENT_OUTPUT_ALREADY_EXISTS");
  } catch (error) {
    if (
      error instanceof Error &&
      error.message === "TERMIX_PERMISSION_AUDIT_AGENT_OUTPUT_ALREADY_EXISTS"
    ) {
      throw error;
    }
...
async function main(): Promise<void> {
  ...
  if (sha256Bytes(bundleCanonicalJson) !== invocation.bundleSha256) {
    throw new Error("TERMIX_PERMISSION_AUDIT_AGENT_INPUT_DIGEST_MISMATCH");
  }
  ...
  const message =
    error instanceof Error && /^[A-Z0-9_]+$/u.test(error.message)
      ...
      : error instanceof Error
        ...
        : "Error";
  process.stderr.write(`TermiX permission audit agent runner failed: ${message}\n`);
  process.exitCode = 1;
});
