...
} from "../packages/benchmarks/src/protectedFinalReport";
...
const INPUT_PAIR_PREFIX = "evidence/termix/pairs/";
const REVIEW_RECORD_PREFIX = "evidence/termix/reviews/independent/";
const REVIEW_PACKET_PREFIX = "evidence/termix/reviewer-packets/";
...
function validateEvidencePath(value: string): string {
  if (
    ...
    fail("TERMIX_FINAL_REVIEW_EVIDENCE_PATH_INVALID");
  }
  return value;
}
...
async function verifyPacketContract(packet: {
  readonly reviewContract: {
    readonly runtimeFiles: readonly {
  ...
  for (const runtimeFile of packet.reviewContract.runtimeFiles) {
    const bytes = await readExactTracked(validateEvidencePath(runtimeFile.path));
    if (sha256Bytes(bytes) !== runtimeFile.sha256) fail("TERMIX_FINAL_REVIEW_CONTRACT_DRIFT");
  }
}
...
async function buildSources(sources: readonly SourceInvocation[]) {
  return Promise.all(
    sources.map(async (source) => {
      ...
      const inputPairPath = validatePath(adjudication.inputPairPath, INPUT_PAIR_PREFIX);
      const inputPairBytes = await readExactTracked(inputPairPath);
      if (sha256Bytes(inputPairBytes) !== adjudication.inputPairBytesSha256) {
        fail("TERMIX_FINAL_INPUT_PAIR_BYTES_DRIFT");
      }
      const inputPair = parseJson(inputPairBytes, "TERMIX_FINAL_INPUT_PAIR_JSON_INVALID");

      const reviewRecordPath = validatePath(adjudication.reviewRecordPath, REVIEW_RECORD_PREFIX);
      const reviewRecordBytes = await readExactTracked(reviewRecordPath);
      if (sha256Bytes(reviewRecordBytes) !== adjudication.reviewRecordBytesSha256) {
        fail("TERMIX_FINAL_REVIEW_RECORD_BYTES_DRIFT");
      }
      const reviewRecord = TermixIndependentReviewRecordSchema.parse(
        parseJson(reviewRecordBytes, "TERMIX_FINAL_REVIEW_RECORD_JSON_INVALID")
      );

      const packetPath = validatePath(adjudication.packetPath, REVIEW_PACKET_PREFIX);
      if (gitText(["log", "-1", "--format=%H", "--", packetPath]) !== reviewRecord.reviewedCommit) {
        fail("TERMIX_FINAL_REVIEWED_COMMIT_MISMATCH");
      }
      const packetBytes = await readExactTracked(packetPath);
      ...
      const packetTask = packet.tasks.find(({ taskId }) => taskId === source.taskId);
      const reviewTask = reviewRecord.tasks.find(({ taskId }) => taskId === source.taskId);
      if (
        packetTask === undefined ||
        reviewTask === undefined ||
        packet.packetId !== reviewRecord.packetId ||
        reviewRecord.packetPath !== packetPath ||
        reviewRecord.packetBytesSha256 !== sha256Bytes(packetBytes) ||
        packet.reviewContract.reviewRecordPath !== reviewRecordPath ||
        packetTask.inputPairPath !== inputPairPath ||
        packetTask.inputPairBytesSha256 !== sha256Bytes(inputPairBytes) ||
        packetTask.inputPairLogicalSha256 !== adjudication.inputPairLogicalSha256 ||
        packetTask.declarationSha256 !== adjudication.declarationSha256 ||
        packetTask.reviewerMustProduce.verifiedPairPath !== source.pairPath ||
        packetTask.reviewerMustProduce.adjudicationPath !== source.adjudicationPath ||
        canonicalJson(packetTask.evidence) !== canonicalJson(reviewTask.evidence)
      ) {
        fail("TERMIX_FINAL_REVIEW_PACKET_BINDING_MISMATCH");
      }
      await verifyReviewEvidence(reviewTask.evidence);
      assertTermixAdjudicationBinding(inputPair, pair, reviewRecord, adjudication);
