...
const PREFIX = "evidence/termix/declarations/venus-health/";
...
function parseArguments(args) {
  ...
  if (
    normalized.length !== 5 ||
    normalized[0] !== "--capture-exact-venus-health-run-order" ||
    ...
    fail("TERMIX_VENUS_ORDER_ARGUMENTS_INVALID");
  }
...
async function readCommittedDeclaration(path) {
  ...
  if ((await realpath(absolute)) !== absolute) fail("TERMIX_VENUS_ORDER_DECLARATION_UNTRUSTED");
  ...
  if (!working.equals(committed)) fail("TERMIX_VENUS_ORDER_DECLARATION_NOT_COMMITTED");
  let parsed;
  try {
    parsed = JSON.parse(working.toString("utf8"));
  } catch {
    fail("TERMIX_VENUS_ORDER_DECLARATION_JSON_INVALID");
  }
  if (
    ...
    parsed.registeredAgent?.agentId !== "1828" ||
    parsed.publicReplaySelection?.ownershipClaimed !== false ||
    parsed.publicReplaySelection?.executionAuthorityClaimed !== false ||
    parsed.sourceWindow?.firstBlockNumber !== "125563831" ||
    parsed.sourceWindow?.lastBlockNumber !== "125564152" ||
    ...
    parsed.claims?.intervention !== false
  ) {
    fail("TERMIX_VENUS_ORDER_DECLARATION_INVALID");
  }
  return parsed;
}
...
async function rpc(provider, method, params, id) {
  ...
  if (!response.ok) fail("TERMIX_VENUS_ORDER_RPC_HTTP_FAILED");
  const body = await response.text();
  if (Buffer.byteLength(body) > 1_000_000) fail("TERMIX_VENUS_ORDER_RPC_RESPONSE_TOO_LARGE");
  let parsed;
  try {
    parsed = JSON.parse(body);
  } catch {
    fail("TERMIX_VENUS_ORDER_RPC_JSON_INVALID");
  }
  if (
    typeof parsed !== "object" ||
    parsed === null ||
    ...
    fail("TERMIX_VENUS_ORDER_RPC_ENVELOPE_INVALID");
  }
...
function blockResult(value, target) {
  if (
    ...
    fail("TERMIX_VENUS_ORDER_BLOCK_INVALID");
  }
  if (quantity(value.number, "TERMIX_VENUS_ORDER_BLOCK_NUMBER_INVALID") !== target) {
    fail("TERMIX_VENUS_ORDER_BLOCK_MISMATCH");
  }
  quantity(value.timestamp, "TERMIX_VENUS_ORDER_BLOCK_TIMESTAMP_INVALID");
...
async function observe(provider, target) {
  ...
  if (chain.response.result !== "0x61") fail("TERMIX_VENUS_ORDER_CHAIN_MISMATCH");
  const head = await rpc(provider, "eth_blockNumber", [], `${provider.id}-head`);
  const headNumber = quantity(head.response.result, "TERMIX_VENUS_ORDER_HEAD_INVALID");
  if (headNumber < target + 12n) fail("TERMIX_VENUS_ORDER_NOT_FINALIZED");
...
async function main() {
  ...
  verifyTermixPublishedReleaseState({
    ...
    protectedPaths: [
      ...
      "agents/healthFactorGuardianAgent/app/agent/src/venusExactWindow.ts",
      "scripts/capture-termix-venus-health-run-order.mjs",
      "scripts/freeze-termix-venus-health-declaration.ts",
      "scripts/termix-release-state.mjs",
      "scripts/termix-typescript-loader.mjs"
    ],
    errorPrefix: "TERMIX_VENUS_ORDER"
  });
  ...
  if (declaration.sourceCommitSha !== options.sourceCommitSha) {
    fail("TERMIX_VENUS_ORDER_SOURCE_COMMIT_MISMATCH");
  }
  ...
  if (
    ...
    fail("TERMIX_VENUS_ORDER_PROVIDER_DISAGREEMENT");
  }
  ...
  const message =
    error instanceof Error && /^[A-Z0-9_]+$/u.test(error.message)
      ...
      : error instanceof Error
        ...
        : "Error";
  process.stderr.write(`TermiX Venus run-order capture failed: ${message}\n`);
  process.exitCode = 1;
});
