...
Updated: 2026-09-05. This is an internal pre-submission control sheet derived from the official Smart Money Era page review recorded in [`research.md`](./research.md), the bounded [submission audit](../evidence/submission/hackathon-submission-audit-2026-09-01.json) and the current [score strategy](./hackathon-score-strategy.md). It is not a judging result, partner endorsement, eligibility decision, or submission receipt. Every “current proof” item must remain independently inspectable; every open item blocks the corresponding claim.
...
| ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| End-to-end journey                 | Landing, mandate-selected live evidence, four registered dossiers, direct Studio handoff and public A2A analysis activation with run ID/time, one-grant Session Control, historical Mission Control, `/proof`, and one controlled chain-97 LP position are implemented                                                    | Release `f25a67d` retains the exact 4/4 public journey with 20 facts, eight PNG hashes and four completed runs; keep capital execution and testnet eligibility separate            | 0:00–4:05               |
| Decision-useful live data          | The marketplace directly reads five bounded category facts from the controlled Pancake testnet position for LP/Grid, the official Lista mainnet API for Yield, or an unrelated public Venus testnet replay for Health; block/retrieval time, methodology, freshness and unknowns remain visible, with no fixture fallback | The v2 release capture observed all four sources available; keep raw state separate from recommendation, performance, execution authority and receipt-linked outcome/cost evidence | 0:35–1:25 and 2:30–3:30 |
| Four first-class categories        | LP, grid, yield, and health each have a selected live-evidence view, public analyzer, category contract, dossier, configuration surface, exact Agent Card skill, and finalized BSC-testnet ERC-8004 identity (`1825`–`1828`)                                                                                              | Keep all four public through judging and show equal current-evidence depth without implying equal execution history                                                                | 4:35–5:00               |
...
| Production credibility             | Durable custom-domain deployment, PM2 supervision, frozen base rollback/restoration, current carrier `1282910` two-resolver/five-TLS-host/11-response rehearsal, product ancestor `f25a67d` richer 4/4 journey capture, and a decoded 325.014-second MiMo demo published at [YouTube](https://youtu.be/ron927GeVXI)       | Retain independent uptime/paging evidence and owner-confirmed separate-viewer playback                                                                                             | Before submission       |
...
| Marketplace quality — 20%        | Public four-category UI, verified identity/hire layers, one-grant control model, proof room and retained zero-instruction analysis-service journey | Capital-bearing hire remains unavailable; owner clean-room playback remains |
...
| ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| Public product     | Carrier `1282910` retains DNS 5/5, TLS 5/5 and HTTP 11/11; ancestor `f25a67d` retains 4 available evidence panels plus 4 completed runs                         | Independent uptime/paging remains unavailable                                          |
...
| Source repository  | Visibility is `PUBLIC`; anonymous page/API/raw README/exact commit/demo page/raw video returned `200`                                                           | Recheck immediately before submission and during judging; bounded access is not uptime |
| Proof room         | Public seven-gate ledger                                                                                                                                        | All seven gates verified by final artifacts                                            |
| Demo               | [Public YouTube video](https://youtu.be/ron927GeVXI), retained six-scene 325.014-second MiMo MP4, and separate-process copy/hash/full-decode/public-scene check | Owner-confirmed narration/presentation playback                                        |
...
Submission-flow watch: as rechecked on 2026-09-01, the canonical CMS `submit_project_url` still
leads to the one-page `Build the Era Hackathon Registration` form. Because this is the official
submit-project field, use it as the current entry flow rather than waiting for an unannounced
replacement. It contains project/repository/Additional Notes fields but no product/demo/evidence
