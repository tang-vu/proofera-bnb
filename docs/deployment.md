...
Updated: 2026-08-15. The PTA fixture is finalized on BSC Testnet; no pool or mainnet action exists. Historical one-off releases stopped at exact generation-1, generation-2, and generation-3 claim-only states without worker start, custody, signature, submission, receipt, pool, or liquidity. Current recovery generation 4 requires its own committed/pushed identity, new exact audits and policy, an invocation-A fence/restart, distinct snapshot B, and separate owner-v7 TTY confirmation.
...
Recovery generation 4 has exactly two invocation roles. Invocation A requires the immutable generation-1 and generation-2 fences and exact expired `1,362`-byte generation-3 claim with raw SHA-256 `0x7ff780a8f0ac1a1f8ff7bced5d858259f918cdb1891c684aa208b6bca31c9585`, exact-empty generation-4 plus predecessor/active submission state, and snapshot A; it rereads six namespaces, appends/rereads generation-3 slot 2, then stops `PREDECESSOR_FENCE_RECORDED_RESTART_REQUIRED`. Invocation B must be a separate process, admit a new policy through TTY v5, obtain snapshot B strictly after the fence, and only then enter owner-v7 ceremony. Any later durable evidence, contamination, path/ACL ambiguity, race, or unknown append blocks.

Generation 4 uses `%LOCALAPPDATA%\ProofEra\operations\bsc-testnet-pta-wbnb-pool-v4` for active signing. Generation-1, generation-2, and generation-3 namespaces remain immutable predecessors. Submission-v2 is a strict read-only predecessor probe; active submission uses `bsc-testnet-pta-wbnb-pool-submission-v3` with v5 journal records. All six namespaces are read before authority. Retained started/unknown state reconciles only; no resend/replacement is possible.
...
This runbook supplies no triplet or canonical policy for generation 4. Historical triplets/policies and owner-v4/v5/v6 confirmations are non-authorizing. It records no generation-4 owner-v7 confirmation, signature, send, receipt, pool, or liquidity.
...
Pool initialization remains a separate write requiring fresh state, exact generation-4 release audits/triplet/policy, and owner-v7 authorization in invocation B. Historical decisions and confirmations close no such gate. Initialization adds no liquidity; any LP mint remains a second separately reviewed and approved transaction.
...
3. Commit and push generation 4, obtain new exact audits and commit/tree/runtime-manifest triplet, and invoke only the absolute PowerShell phase-minus-one command. Invocation A may only fence generation 3 and require restart. Invocation B admits the matching policy through TTY v5, obtains later snapshot B, and then requires fresh owner-v7 authorization. Reconcile any initializer receipt before preparing a separately approved LP mint.
