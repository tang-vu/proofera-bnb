...
Updated: 2026-08-25. The PTA fixture is finalized on BSC Testnet; no pool or mainnet action exists. Historical releases through generation 3 stopped at claim-only states. Generations 4, 5, and 6 later accepted their exact owner confirmations and durably ended `failed_before_worker`, still without worker start, custody-secret access, signature, submission, receipt, pool, or liquidity. Current recovery generation 7 pins its only initializer to sender nonce `9`; it requires its own committed/pushed identity, new exact audits and policy, a fresh envelope after the exact generation-6 terminal, and separate owner-v10 TTY confirmation.
...
Recovery generation 7 binds the generation-6 terminal and inherited generation-5 terminal, requires submission-v5 to have no retained transaction state, and admits only a fresh distinct envelope through policy/runtime-instantiation v7, TTY v8, and owner-v10. Signing/authorized intent/broadcast are v7. Active signing uses `%LOCALAPPDATA%\ProofEra\operations\bsc-testnet-pta-wbnb-pool-v7`; active submission uses `bsc-testnet-pta-wbnb-pool-submission-v6` with v8 records and durable owner policy v8. All twelve signing/submission namespaces are read before authority. Its exact transaction nonce remains `9`; timing caps remain `300/240/120/60/60/20/30` seconds. Retained started/unknown state reconciles only; no resend/replacement is possible.
...
This runbook supplies no release-specific triplet or canonical policy for generation 7. Historical triplets/policies and owner-v4/v5/v6/v7/v8/v9 confirmations are non-authorizing. It records no generation-7 owner-v10 confirmation, signature, send, receipt, pool, or liquidity.

The command exits successfully only after a validated durable `confirmed` result. `reconciliation_pending`, `reverted`, blocked and do-not-retry results exit nonzero. Generation 7 has no Invocation-A restart path; its exact generation-6 terminal is immutable predecessor evidence, not authority. For `reconciliation_pending`, rerun the same command only to invoke recovery-first receipt reconciliation; the journals ensure that this cannot resend or replace the transaction. A reverted or other terminal result must not be retried.

Pool initialization remains a separate write requiring fresh state, exact generation-7 release audits/triplet/policy, and owner-v10 authorization. Historical decisions and confirmations close no such gate. Initialization adds no liquidity; any LP mint remains a second separately reviewed and approved transaction.
...
3. Commit and push generation 7, obtain new exact audits and commit/tree/runtime-manifest triplet, and invoke only the absolute PowerShell phase-minus-one command. Bind the exact generation-6 terminal, admit the matching policy through TTY v8 on a fresh later envelope, and require fresh owner-v10 authorization. Reconcile any initializer receipt before preparing a separately approved LP mint.
