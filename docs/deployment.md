...
Updated: 2026-08-15. The PTA fixture is finalized on BSC Testnet; no pool or mainnet action exists. Historical one-off releases stopped at exact generation-1 and generation-2 claim-only states without worker start, custody, signature, submission, receipt, pool, or liquidity. Current recovery generation 3 requires its own committed/pushed identity, new exact audits and policy, an invocation-A fence/restart, distinct snapshot B, and separate owner-v6 TTY confirmation.
...
Recovery generation 3 has exactly two invocation roles. Invocation A requires the immutable generation-1 fence and exact expired `1,362`-byte generation-2 claim with raw SHA-256 `0x613df995936c3ccfff56e5da5588906f1bd28340ae8297eb08524274b9b8e1c3`, exact-empty generation-3/submission state, and snapshot A; it rereads four namespaces, appends/rereads generation-2 slot 2, then stops `PREDECESSOR_FENCE_RECORDED_RESTART_REQUIRED`. Invocation B must be a separate process, admit a new policy through TTY v4, obtain snapshot B strictly after the fence, and only then enter owner-v6 ceremony. Any later durable evidence, contamination, path/ACL ambiguity, race, or unknown append blocks.

Generation 3 uses `%LOCALAPPDATA%\ProofEra\operations\bsc-testnet-pta-wbnb-pool-v3` for active signing. Generation-1 and generation-2 namespaces remain immutable predecessors. Submission deliberately retains directory `bsc-testnet-pta-wbnb-pool-submission-v2`, but current schema is `bsc_testnet_pta_wbnb_pool_submission_journal_v4` and files end `.v4.json`. All four namespaces are read before authority. Retained started/unknown state reconciles only; no resend/replacement is possible.
...
This runbook supplies no triplet or canonical policy for generation 3. Historical triplets/policies and owner-v4/v5 confirmations are non-authorizing. It records no generation-3 owner-v6 confirmation, signature, send, receipt, pool, or liquidity.

The command exits successfully only after a validated durable `confirmed` result. `reconciliation_pending`, `reverted`, blocked and do-not-retry results exit nonzero. The exact `PREDECESSOR_FENCE_RECORDED_RESTART_REQUIRED` result is the only pre-worker blocked result that calls for a new invocation, and that new invocation must begin from the retained fence; it cannot reuse authority from invocation A. For `reconciliation_pending`, rerun the same command only to invoke recovery-first receipt reconciliation; the journals ensure that this cannot resend or replace the transaction. A reverted or other terminal result must not be retried.

Pool initialization remains a separate write requiring fresh state, exact generation-3 release audits/triplet/policy, and owner-v6 authorization in invocation B. Historical decisions and confirmations close no such gate. Initialization adds no liquidity; any LP mint remains a second separately reviewed and approved transaction.
...
3. Commit and push generation 3, obtain new exact audits and commit/tree/runtime-manifest triplet, and invoke only the absolute PowerShell phase-minus-one command. Invocation A may only fence generation 2 and require restart. Invocation B admits the matching policy through TTY v4, obtains later snapshot B, and then requires fresh owner-v6 authorization. Reconcile any initializer receipt before preparing a separately approved LP mint.
