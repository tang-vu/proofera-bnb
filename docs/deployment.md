...
This runbook supplies no release-specific triplet or canonical policy for generation 10. Historical triplets/policies and owner-v4 through owner-v12 confirmations are non-authorizing for generation 10. It records a bounded local generation-9 signature state but no send, receipt, pool, or liquidity.
...
Pool initialization remains a separate write requiring fresh state, exact generation-10 release audits/triplet/policy, and owner-v13 authorization for the existing signature. Historical decisions and confirmations close no such gate. Initialization adds no liquidity; any LP mint remains a second separately reviewed and approved transaction.
...
3. Commit and push generation 10, obtain two new exact owner-designated internal reviews and the commit/tree/runtime-manifest triplet, and invoke only the absolute PowerShell phase-minus-one command. Admit the matching existing-signature policy through TTY v11 and require fresh owner-v13 authorization. Do not unlock custody or create another signature. If durable start exists, reconcile only; require a validated initializer receipt before preparing a separately approved LP mint.
