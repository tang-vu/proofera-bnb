...
The fixed two-RPC coordinator uses a common finalized EIP-1898 checkpoint, verifies runtime/proxy slots/protocol bindings and latest/pending race state, and requires exact simulation plus balance/gas caps. A reviewed scaffold now implements the fixed transaction protocol, authorization validator, signer core, Windows worker and append-only operation journal at `%LOCALAPPDATA%\ProofEra\operations\bsc-testnet-pta-wbnb-pool-v1`. Journal directory/files require protected current-user ACLs. Its receipt self-hash is integrity-only and cannot mint authority. The production authority gate, signer core and worker factory all deliberately fail closed with `PRODUCTION_AUTHORIZATION_UNAVAILABLE`. No production composition, authenticated post-claim RPC issuer, broadcast/receipt runner, custody read, signature, transaction or onchain write exists. Eight focused integration files pass 70 tests; that is implementation evidence only, not deployment or transaction evidence.

Pool initialization remains a separate write requiring fresh state, authenticated independent review, exact owner authorization, an independently reviewed production composition and post-claim RPC capability, plus broadcast, receipt and post-state reconciliation. The local scaffold closes none of those external or production gates. Initialization adds no liquidity. A later LP plan may propose caps of at most `1,000 PTA` plus `0.001 WBNB`, but that proposal has no approval, funding, wrapping, allowance, calldata, position, or receipt. LP minting requires its own review and separate explicit approval after pool, oracle, liquidity, ownership, slippage, deadline, authority, and revoke gates close.
