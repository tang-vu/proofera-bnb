...
The direct initializer selector is also locally bound to the exact Position Manager source/artifact/runtime and factory/deployer/pool call graph. It remains non-authorizing because its canonical bytes have not been published at a stable content-addressed locator, independently re-fetched, or approved by an authenticated independent reviewer. The fixed two-RPC coordinator uses a common finalized EIP-1898 checkpoint, verifies runtime/proxy slots/protocol bindings and latest/pending race state, and requires exact simulation plus balance/gas caps; 25 focused tests pass. A separate one-shot descriptor only specifies future interfaces. No pool-initialization authorization-receipt issuer, durable journal, signer, broadcast runner, or receipt reconciler is implemented.

Pool initialization remains a separate write requiring fresh state, exact authorization, an implemented durable one-shot claim and custody-isolated signer, a post-claim pending-state recheck, receipt and post-state reconciliation. It adds no liquidity. A later LP plan may propose caps of at most `1,000 PTA` plus `0.001 WBNB`, but that proposal has no approval, funding, wrapping, allowance, calldata, position, or receipt. LP minting requires its own review and separate explicit approval after pool, oracle, liquidity, ownership, slippage, deadline, authority, and revoke gates close.
