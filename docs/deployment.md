...
A later submission/reconciliation scaffold validates the one exact signed transaction and fresh pre-submit dual-RPC snapshot; requires winner-only durable `submission_started`; never resends or creates a replacement after a send starts; and validates dual-provider transaction, receipt, expected logs and EIP-1898 post-state. Fixed official-RPC normalization, a separate append-only submission journal and an unwired composition exist locally, but the runner, native worker issuer and broadcaster hard-block with `PRODUCTION_AUTHORIZATION_UNAVAILABLE` before custody/sign/send. Future production wiring still requires a same-process two-phase exact owner challenge, closure-private one-consume capability, terminal pre-send reread, restart-first reconciliation and receipt-to-finality ancestry proof. No signature, send, receipt, pool or liquidity exists. These are implementation tests only, not deployment, authority, or transaction evidence.

Pool initialization remains a separate write requiring fresh state, a new owner-designated distinct-agent technical decision for any changed production release, exact owner authorization, an independently reviewed production RPC/composition and durable submission journal, plus broadcaster and receipt/post-state reconciliation. The current decision covers only the nonexecuting `bc7000e` subject; the old-scope unsent request and test-only cores close no production or owner-authorization gate. Initialization adds no liquidity. A later LP plan may propose caps of at most `1,000 PTA` plus `0.001 WBNB`, but that proposal has no approval, funding, wrapping, allowance, calldata, position, or receipt. LP minting requires its own review and separate explicit approval after pool, oracle, liquidity, ownership, slippage, deadline, authority, and revoke gates close.
