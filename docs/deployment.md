...
The separate `contracts/testnet-hire-receipt` package is an undeployed evidence mechanism for the three TermiX agent lanes. Its constructor is chain-97-only and binds the immutable ERC-8004 registry. `hire` requires a nonzero, at-most-`0.01 tBNB` payment, a fresh expiry no more than seven days away, a nonzero task hash and a never-used engagement ID; it resolves the registered owner, records the receipt hash before interaction, atomically forwards the full payment, and emits the complete receipt. There is no administrator, escrow balance, arbitrary recipient or execution permission. Routine compilation/tests and the preparation script are offline. The separate operator runner is exact-approval/preparation/release gated, probes both fixed RPCs before every signature, uses only the pinned current-user DPAPI custody path, journals before broadcast, and halts on uncertain outcomes; it is not exposed through a package script or web route. The read-only `capture:termix:hires` path accepts only four transaction hashes and emits create-only evidence after two-provider transaction/receipt/block agreement, 12-block finality, exact runtime, owner, event and stored-receipt verification. Deployment plus three `0.00001 tBNB` hires require a separate exact approval after fresh nonce, predicted-address code, expiry, calldata, gas and balance checks. Until finalized receipts are independently collected, this is preparation and must not be described as a live marketplace hire.
