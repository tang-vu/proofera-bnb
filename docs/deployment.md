...
This key is only for bounded BSC testnet deployment preparation. Every consuming command must fail closed unless the provider and transaction chain ID are exactly `97`; an EVM key itself is not chain-bound. It is not the browser Altana passkey, autonomous session signer, user capital wallet or a mainnet key. Before its first use, add a reviewed signer loader that never exposes the DPAPI password/private key through argv, environment variables, logs or the browser, then prepare an unsigned transaction, pin sender/nonce/gas/fee/value/target/calldata/deadline, and request explicit approval before faucet funding or broadcast.
