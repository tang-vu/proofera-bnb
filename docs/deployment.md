...
Updated: 2026-08-23. The PTA fixture is finalized on BSC Testnet; no pool or mainnet action exists. Historical releases through generation 3 stopped at claim-only states. A later one-off local generation-4 run accepted owner-v7 confirmation and durably recorded `failed_before_worker`, still without worker start, custody-secret access, signature, submission, receipt, pool, or liquidity. Unrelated retained chain-97 work later advanced the shared test EOA. Current recovery generation 5 is unused and now pins its only initializer to sender nonce `9`; it still requires its own committed/pushed identity, new exact audits and policy, a fresh envelope after the exact generation-4 terminal, and separate owner-v8 TTY confirmation.
...
Recovery generation 5 binds that exact terminal directly and separately requires exact-empty submission-v3. There is no Invocation A, no new no-effect proof, and no reusable fence snapshot. It admits only a fresh distinct envelope after the terminal through policy/runtime-instantiation v5, TTY v6, and owner-v8. Signing/authorized intent/broadcast are v5; active submission is v4 with schema/files v6. Its exact transaction nonce is `9`; a 2026-08-23 non-retained two-RPC observation motivated this rebase after unrelated chain-97 writes, but only fresh in-ceremony agreement may authorize progress. Its envelope/owner-cap/authority/preclaim/recheck caps are exactly `300/240/120/60/60/20/30` seconds; the owner deadline is further bounded by preserving the full 120-second authority reserve. Any nonce drift, durable evidence, contamination, path/ACL ambiguity, race, or unknown append blocks.
...
This runbook supplies no release-specific triplet or canonical policy for the rebased generation 5. Historical triplets/policies, nonce-`1` technical decisions and owner-v4/v5/v6/v7 confirmations are non-authorizing. It records no generation-5 nonce-`9` owner-v8 confirmation, signature, send, receipt, pool, or liquidity.
