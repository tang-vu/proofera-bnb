...
Updated: 2026-08-25. The PTA fixture is finalized on BSC Testnet; no pool or mainnet action exists. Historical releases through generation 3 stopped at claim-only states. Generation 4 and generation 5 each later accepted their exact owner confirmation and durably ended `failed_before_worker`, still without worker start, custody-secret access, signature, submission, receipt, pool, or liquidity. Current recovery generation 6 pins its only initializer to sender nonce `9`; it requires its own committed/pushed identity, new exact audits and policy, a fresh envelope after the exact generation-5 terminal, and separate owner-v9 TTY confirmation.
...
Generation 5 is closed historical state. Its exact local-only signing records are `01-claim.v5.json` (`1,364` bytes, SHA-256 `0x0d76c35b7d6cdec488b8b79dafcefacc597c79f057fe722a2202d284515017f1`) and `02-transition.v5.json` (`1,383` bytes, SHA-256 `0x39a6295f3f816cb5bba6c8c3be11982bcafa45847608e1150de950738217c8c9`). The terminal code is `POST_CLAIM_RECHECK_OUTCOME_UNKNOWN`; no worker, custody unlock, serialized transaction, transaction hash, submission, or receipt exists. Recovery generation 6 binds that terminal and its inherited generation-4 terminal, requires submission-v4 to have no retained transaction state, and admits only a fresh distinct envelope through policy/runtime-instantiation v6, TTY v7, and owner-v9. Signing/authorized intent/broadcast are v6. Its exact transaction nonce remains `9`, and only fresh in-ceremony agreement may authorize progress. Its timing caps remain exactly `300/240/120/60/60/20/30` seconds. Any nonce drift, durable evidence, contamination, path/ACL ambiguity, race, or unknown append blocks.

Generation 6 uses `%LOCALAPPDATA%\ProofEra\operations\bsc-testnet-pta-wbnb-pool-v6` for active signing. Generation-1 through generation-5 signing namespaces remain immutable predecessors. Submission-v2, submission-v3, and submission-v4 are strict read-only predecessor probes; active submission uses `bsc-testnet-pta-wbnb-pool-submission-v5` with v7 journal records and durable owner policy v7. All ten namespaces are read before authority. Retained started/unknown state reconciles only; no resend/replacement is possible.
...
This runbook supplies no release-specific triplet or canonical policy for generation 6. Historical triplets/policies and owner-v4/v5/v6/v7/v8 confirmations are non-authorizing. It records no generation-6 owner-v9 confirmation, signature, send, receipt, pool, or liquidity.

The command exits successfully only after a validated durable `confirmed` result. `reconciliation_pending`, `reverted`, blocked and do-not-retry results exit nonzero. Generation 6 has no Invocation-A restart path; its exact generation-5 terminal is immutable predecessor evidence, not authority. For `reconciliation_pending`, rerun the same command only to invoke recovery-first receipt reconciliation; the journals ensure that this cannot resend or replace the transaction. A reverted or other terminal result must not be retried.

Pool initialization remains a separate write requiring fresh state, exact generation-6 release audits/triplet/policy, and owner-v9 authorization. Historical decisions and confirmations close no such gate. Initialization adds no liquidity; any LP mint remains a second separately reviewed and approved transaction.
...
3. Commit and push generation 6, obtain new exact audits and commit/tree/runtime-manifest triplet, and invoke only the absolute PowerShell phase-minus-one command. Bind the exact generation-5 terminal, admit the matching policy through TTY v7 on a fresh later envelope, and require fresh owner-v9 authorization. Reconcile any initializer receipt before preparing a separately approved LP mint.
