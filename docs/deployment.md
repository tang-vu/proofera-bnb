...
Updated: 2026-08-26. The PTA fixture is finalized on BSC Testnet; no pool or mainnet action exists. Historical releases through generation 3 stopped at claim-only states. Generations 4 through 8 accepted their exact owner confirmations and durably ended `failed_before_worker`, still without worker start, custody-secret access, signature, submission, receipt, pool, or liquidity. Generation 8's exact terminal code is `GAS_POLICY_VIOLATION`. Current recovery generation 9 pins its only initializer to sender nonce `9`; it requires its own committed/pushed identity, new exact audits and policy, a fresh envelope after the exact generation-8 terminal, and separate owner-v12 TTY confirmation.
...
Generation 8 is closed historical state. Release `08f0357f1281c2289a1a0db9637e8fb082cb6900` accepted exact owner-v11 bytes and retained `01-claim.v8.json` (`1,364` bytes, SHA-256 `0x5a85737428a4bbd06459ceab52d6096fba74aa1c002de31a24c942ff9f3954f6`) plus `02-transition.v8.json` (`1,369` bytes, SHA-256 `0x3210fd8ab08c2282a5da1aeb426984592fed9a5b3a6832ac7d60991baaf4fc6d`). The terminal code is `GAS_POLICY_VIOLATION`, with outcome digest `0x15b8bd2046fdac833c932d21deea39e7901bb97398622ad03e7625167e19d469`; fresh estimate `5,012,641` exceeded the then-authorized `5,000,000` maximum. No worker, custody unlock, serialized transaction, transaction hash, submission, or receipt exists.

Recovery generation 9 binds the generation-8 terminal and inherited generation-7 terminal, requires submission-v7 to have no retained transaction state, and admits only a fresh distinct envelope through policy/runtime-instantiation v9, TTY v10, and owner-v12. Signing/authorized intent/broadcast are v9. Active signing uses `%LOCALAPPDATA%\ProofEra\operations\bsc-testnet-pta-wbnb-pool-v9`; active submission uses `bsc-testnet-pta-wbnb-pool-submission-v8` with v10 records and durable owner policy v10. All sixteen signing/submission namespaces are read before authority. The fresh estimate cap is `5,500,000`, and accepted estimates bind the fixed `6,600,000` signed gas limit; gas-price and total-cost caps remain unchanged. Post-claim RPC requests are ordered dual-provider pairs with at most one in flight per endpoint and no retry. Its exact transaction nonce remains `9`; timing caps remain `300/240/120/60/60/20/30` seconds. Retained started/unknown state reconciles only; no resend/replacement is possible.
...
This runbook supplies no release-specific triplet or canonical policy for generation 9. Historical triplets/policies and owner-v4 through owner-v11 confirmations are non-authorizing. It records no generation-9 owner-v12 confirmation, signature, send, receipt, pool, or liquidity.

The command exits successfully only after a validated durable `confirmed` result. `reconciliation_pending`, `reverted`, blocked and do-not-retry results exit nonzero. Generation 9 has no Invocation-A restart path; its exact generation-8 terminal is immutable predecessor evidence, not authority. For `reconciliation_pending`, rerun the same command only to invoke recovery-first receipt reconciliation; the journals ensure that this cannot resend or replace the transaction. A reverted or other terminal result must not be retried.

Pool initialization remains a separate write requiring fresh state, exact generation-9 release audits/triplet/policy, and owner-v12 authorization. Historical decisions and confirmations close no such gate. Initialization adds no liquidity; any LP mint remains a second separately reviewed and approved transaction.
...
3. Commit and push generation 9, obtain new exact audits and commit/tree/runtime-manifest triplet, and invoke only the absolute PowerShell phase-minus-one command. Bind the exact generation-8 terminal, admit the matching policy through TTY v10 on a fresh later envelope, and require fresh owner-v12 authorization. Reconcile any initializer receipt before preparing a separately approved LP mint.
