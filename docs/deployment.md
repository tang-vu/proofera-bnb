...
A deterministic [external-review request bundle](../evidence/development/pancake-v3-pta-wbnb-external-review-request-2026-08-13.json) pins the exact direct-call subject to source commit `00f21c405881a5dc320bddf3c757ba13599b1e71`, exactly its enumerated eight implementation files, the revision-pinned Gist, and retained evidence hashes. It has not been sent and has no recipient, reviewer, provisioned reviewer identity, or Sigstore authentication evidence. Its SHA-256 values provide unkeyed integrity only. It excludes the later post-claim and submission/reconciliation files. Even a future authenticated decision would cover only that fixed old subject—not the current or a future activation release—and cannot substitute for exact owner authorization. Activation requires a new request and authenticated independent decision pinned to the complete exact release.

The fixed two-RPC coordinator uses a common finalized EIP-1898 checkpoint, verifies runtime/proxy slots/protocol bindings and latest/pending race state, and requires exact simulation plus balance/gas caps. A reviewed scaffold implements the fixed transaction protocol, authorization validator, signer core, Windows worker and append-only signing journal at `%LOCALAPPDATA%\ProofEra\operations\bsc-testnet-pta-wbnb-pool-v1`. Journal directory/files require protected current-user ACLs. Its receipt self-hash is integrity-only and cannot mint authority. The production authority gate, signer core and worker factory all fail closed with `PRODUCTION_AUTHORIZATION_UNAVAILABLE`. A separate test-only post-claim core requires an authenticated exact intent before any read and repeats fixed dual-RPC common-finalized EIP-1898, nonce, pool/candidate, simulation, balance, gas and expiry checks. Its production constructor is likewise hard-blocked.

A later test-only submission/reconciliation core uses injected ports only. It validates the one exact signed transaction and fresh pre-submit dual-RPC snapshot; requires winner-only durable `submission_started`; never resends or creates a replacement after a send starts; and accepts a terminal result only after both providers agree on the exact transaction, common-finalized canonical receipt block, expected `PoolCreated`/`Initialize` logs, and exact EIP-1898 pool post-state. Its production factory fails with `PRODUCTION_AUTHORIZATION_UNAVAILABLE`. No production RPC adapter, durable submission journal, broadcaster, reconciler, signature, send, receipt, pool or liquidity exists. These are implementation tests only, not deployment, authority, or transaction evidence.

Pool initialization remains a separate write requiring fresh state, a new authenticated independent review pinned to the complete exact activation release, exact owner authorization, an independently reviewed production RPC/composition and durable submission journal, plus broadcaster and receipt/post-state reconciliation. The old-scope unsent request and test-only cores close none of those external or production gates. Initialization adds no liquidity. A later LP plan may propose caps of at most `1,000 PTA` plus `0.001 WBNB`, but that proposal has no approval, funding, wrapping, allowance, calldata, position, or receipt. LP minting requires its own review and separate explicit approval after pool, oracle, liquidity, ownership, slippage, deadline, authority, and revoke gates close.
