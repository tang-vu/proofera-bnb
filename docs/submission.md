...
**TermiX:** the strict harness and exactly three active digest-bound protocols exist for LP trading, permission security and Venus health. LP v2 preserves the never-run PTA/WBNB v1 definition, uses the real public USDT/WBNB evidence candidate, separates source chain 56 from ERC-8004/hire chain 97, and has fixed agent/manual lanes plus a create-only production agent CLI. Venus has fixed agent/manual lanes and its own create-only CLI. Each agent lane remains unreachable until registration plus a verified hire receipt passes the outer gate; each manual lane records positive operator-active segments, fixed read-only RPC exchanges and an independently reviewable no-agent declaration. Final declarations, identities, hire receipts, run order and invocations remain `UNBOUND`; every method is `NOT RUN` and publication is false. The final evidence must contain three frozen, reproducible agent-versus-manual runs measuring wall/active time, cost and predeclared output quality.
