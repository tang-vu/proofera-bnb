...
**TermiX:** the strict harness and exactly three digest-bound preregistrations exist for LP trading, permission security, and Venus health. The fixed Venus agent lane now digest-binds its canonical request, public endpoint and configuration, preserves the raw A2A response, and remains unreachable until ERC-8004 registration plus a verified hire receipt pass the outer gate. Final task bindings and the timed command remain `UNBOUND`, both methods are `NOT RUN`, and publication is false; the local digests are change detectors rather than timestamps or results. The final evidence must contain three frozen, reproducible agent-versus-manual runs measuring wall/active time, cost and predeclared output quality.
