...
| Altana state machine         | ACTIVE / AUTHORITY GATED        | Public-descriptor/authority, 16 exact-SDK grant, 68 canonical same-pool PostgreSQL grant-claim, and execute/revoke lifecycle tests pass; both ledgers are host-local deployed and schema/app-verified. A 25-call read-only capture at finalized block `125493138` proves the pinned SDK 0.7.0 BSC-testnet relay capabilities, KeyStore/controller runtime, six relay contract runtimes and registration fee across two RPCs. Final-origin SDK passkey create/recover controls distinguish a locally retained counterfactual wallet from KeyStore recovery, disable recovery once its public descriptor is retained, expose bounded failure codes without claiming authority, and admit only the two SDK-pinned chain-97 browser connect origins through CSP; no successful owner ceremony is claimed. Authenticated worker custody, a completed passkey grant, eligible action, authority and receipt remain absent |
