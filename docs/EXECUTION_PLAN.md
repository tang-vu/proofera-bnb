...
| Fixed test-asset fallback    | DONE locally / UNDEPLOYED          | Chain-97-only fixed-supply PTA plus deterministic PTA/WBNB review-call preparation pass 11 Solidity tests including 256 fuzz runs and 25 Node tests. A separate read-only deployment observer and pure EIP-155 serializer validate the exact 2,947-byte payload/runtime, two-provider state and non-broadcastable signing payload without authorizing signing; no token, pool, price, liquidity or onchain receipt exists                                                                  |
| Testnet deployer custody     | DONE locally / UNFUNDED            | Dedicated address `0x997cD959798F7c925076eaeFF5855C5C2c1e5A49`; Web3 v3 keystore plus DPAPI-wrapped random password remain outside the repo. The custody probe passes 30 focused and 1 opt-in Windows check. Deployment is one-shot pinned to nonce `0` and `0x4ed64525d6fB06b7dA926C683CBD809632C9B4Cc`; the two-official-RPC observation confirmed that state and exact runtime simulation but blocked on balance `0`. No signing API, broadcast, deployment or explorer evidence exists |
...
- Testnet fallback asset: PTA is a locally verified non-economic ERC-20 with no admin or later-mint surface, not a stablecoin or execution guard. Its offline pool tool binds the official chain-97 tuple, token order, fee-500 initializer selector, zero value and arbitrary raw-unit 1:1 seed while emitting no transaction envelope or pool prediction. The separate deployment observer/serializer hard-pins the first deployer nonce and exact CREATE target, but remains non-authorizing and has no funded live envelope. All twelve pool-submission/activation gates remain open. PTA remains undeployed and cannot satisfy pool eligibility until an explicitly approved minimal deployment/pool is independently observed and reviewed.
...
None block further local implementation. The first real PTA deployment is now externally blocked on testnet funding: the official web faucet requires hCaptcha and currently states that `0.002 BNB` on BSC mainnet is needed, while the new deployer has zero mainnet and testnet balance. Use the official Discord/Telegram faucet path with the user's account; do not bypass captcha or fund this disposable test key with real assets. Release also requires the npm-account owner to confirm revocation/rotation of the registry token exposed in prior local tool output and review the account's access/publication history; the token value is not retained here. Other expected requests remain limited to 8004scan Pro quota, durable judging-host credentials, and explicit approval before any mainnet or paid action. The BNB-managed Studio trial lasts only 48 hours and is not a judging-uptime solution.
