...
- Policy-TTY transport uses nonce-bound exact-order ASCII `BEGIN`/`CHUNK`/`END`; generation 6 uses challenge/frame v7. Limits remain line `4,096`, valid worst case `2,618`, total `102,400`, policy `1..65,536`, payload `2,304`, `N <= 38`, and one five-minute deadline. Retired v1-v6 domains, truncation, duplicate/missing/reordered/control/trailing/buffered input reject before policy, RPC, custody, or authority.
- Runtime timing caps are exactly `300/240/120/60/60/20/30` seconds for envelope, owner-entry cap, execution authority, minimum remaining at claim, maximum post-confirmation preclaim, post-recheck reserve, and freshness. The owner-entry deadline is further bounded by preserving the full 120-second authority reserve. Generation 6 binds the exact generation-5 terminal through owner-v9; only an exact match captures `confirmedAt`.
...
- Incident-only recovery generation 6: phase minus one scrubs/pins Process environment and disables QuickEdit before Node. It directly binds the immutable generation-5 terminal and its inherited generation-4 terminal, and separately requires submission-v4 to contain no retained transaction state; there is no Invocation A, no no-effect fence, and no reusable predecessor snapshot. The exact generation-5 claim/terminal pair proves no worker authorization/start/signature, and no submission record or transaction hash exists. It rereads ten namespaces and requires a fresh distinct envelope after the terminal. Policy/runtime-instantiation is v6, TTY is v7, owner command/text/confirmation is v9, signing/intent/broadcast are v6, active signing is v6, and active submission-v5 uses v7 records with durable owner policy v7.
