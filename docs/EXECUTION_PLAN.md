...
| Pancake read/execute adapter | ACTIVE / RECONCILIATION GATED       | Atomic reads, source/selector/calldata validation, release pinning and QuickEdit-off bootstrap pass. A create-only mainnet public-position capture retains exact RPC+A2A evidence of one-tick boundary-risk detection while denying ownership/performance/execution claims. Generation 10 used the sole retained Gen9 signature after TTY v11 policy admission and separate owner-v13 confirmation. Durable start now exists, so every restart is reconciliation-only with no resend/replacement. Both fixed providers observe the exact status-1 receipt/logs, but missing historical EIP-1898 state prevents ProofEra's strict terminal confirmation                                                                                             |
| Fixed test-asset fallback    | DONE / INITIALIZER RECEIPT OBSERVED | Chain-97-only fixed-supply PTA plus deterministic PTA/WBNB construction pass the isolated gates. Exact nonce-`0` deployment transaction `0x0852…68c7` is finalized at `0x4ed64525d6fB06b7dA926C683CBD809632C9B4Cc`; both fixed RPCs now also agree on initializer transaction `0xa24d…4022`, its status-1 receipt and expected event payloads for candidate `0x30b07e82d7181a53Ae2EA98Cd08b6733Ffd831aE`. Missing historical EIP-1898 state prevents strict durable confirmation. No liquidity, oracle history, position or activation receipt exists                                                                                                                                                                                              |
