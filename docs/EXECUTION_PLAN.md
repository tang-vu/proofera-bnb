...
The Windows release topology now fails closed without an immutable build identifier. Its release probe binds the public health response to the exact published commit and requires readiness to be honestly `not_ready`, never `misconfigured`; a PM2 monitor repeats the same marketplace/agent/Card/readiness checks every five minutes. The public `/proof` route renders the exact build, four endpoint skill contracts and the committed seven-gate submission ledger, including blockers and digest-bound artifacts. Commit `c476b01b5eb01d9595232e90cca40dece564e91a` is deployed and its two-resolver/five-TLS-host/eleven-HTTP-response rehearsal is retained at SHA-256 `ba4e8fc1ddc31bdae7240457f1e5ae2029bc4f06fb5e24945e73c733f723becf`. Its final attempt failed closed before network because the old prerequisite accepted only Pancake `verified`. The subsequent evidence-policy revision admits the exact `controlled_outcome_observed` negative result for release-freeze purposes only, requires all three Pancake artifact kinds plus the explicit no-benefit/no-advantage claim, and records that no benefit claim was verified. That revision is not the deployed `c476b01` application release. External alert delivery, rollback exercise, deployed data/worker probes, final release freeze, and a genuinely ready activation path remain open.
