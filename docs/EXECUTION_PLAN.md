...
| Pancake read/execute adapter | ACTIVE / RECOVERY GATED       | Atomic reads, source/selector/calldata validation, release pinning and QuickEdit-off bootstrap pass. A create-only mainnet public-position capture now retains exact RPC+A2A evidence of one-tick boundary-risk detection while denying ownership/performance/execution claims. Generations 5 through 8 ended durably `failed_before_worker` without a signature or submission; generation 8's exact code is `GAS_POLICY_VIOLATION`. Recovery generation 9 binds the exact generation-8 terminal, requires a fresh distinct envelope, TTY v10 policy admission, and separate owner-v12 confirmation. Policy admission and owner confirmation remain distinct runtime gates and cannot be inferred from this plan                                   |
...
- Policy-TTY transport uses nonce-bound exact-order ASCII `BEGIN`/`CHUNK`/`END`; generation 9 uses challenge/frame v10. Limits remain line `4,096`, valid worst case `2,619`, total `102,400`, policy `1..65,536`, payload `2,304`, `N <= 38`, and one five-minute deadline. Retired v1-v9 domains, truncation, duplicate/missing/reordered/control/trailing/buffered input reject before policy, RPC, custody, or authority.
- Runtime timing caps are exactly `300/240/120/60/60/20/30` seconds for envelope, owner-entry cap, execution authority, minimum remaining at claim, maximum post-confirmation preclaim, post-recheck reserve, and freshness. The owner-entry deadline is further bounded by preserving the full 120-second authority reserve. Generation 9 binds the exact generation-8 terminal through owner-v12; only an exact match captures `confirmedAt`.
...
- Append-only recovery generation 9: generation 8 accepted owner-v11 for release `08f0357f1281c2289a1a0db9637e8fb082cb6900` and durably retained claim SHA-256 `0x5a85737428a4bbd06459ceab52d6096fba74aa1c002de31a24c942ff9f3954f6` plus `failed_before_worker` terminal SHA-256 `0x3210fd8ab08c2282a5da1aeb426984592fed9a5b3a6832ac7d60991baaf4fc6d`, code `GAS_POLICY_VIOLATION`, and outcome digest `0x15b8bd2046fdac833c932d21deea39e7901bb97398622ad03e7625167e19d469`. There was no worker authorization/start, custody unlock, signature, submission, transaction hash, or receipt. Generation 9 binds that terminal and inherited generation-7 terminal, separately requires submission-v7 to contain no retained transaction state, rereads sixteen namespaces, and requires a fresh distinct envelope. Policy/runtime-instantiation is v9, TTY is v10, owner command/text/confirmation is v12, signing/intent/broadcast are v9, active signing is v9, and active submission-v8 uses v10 records with durable owner policy v10. The gas estimate cap is `5,500,000`; every accepted estimate binds the fixed `6,600,000` gas limit while price/total-cost caps remain unchanged. Post-claim dual-RPC requests are ordered with at most one in flight per endpoint and no retry; known failures persist their exact code.
