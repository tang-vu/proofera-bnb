...
| Pancake read/execute adapter | ACTIVE / RECOVERY GATED       | Atomic reads, source/selector/calldata validation, release pinning and QuickEdit-off bootstrap pass. A create-only mainnet public-position capture now retains exact RPC+A2A evidence of one-tick boundary-risk detection while denying ownership/performance/execution claims. Generations 5 through 7 ended durably `failed_before_worker` without a signature or submission. Recovery generation 8 binds the exact generation-7 terminal, requires a fresh distinct envelope, TTY v9 policy admission, and separate owner-v11 confirmation. Policy admission and owner confirmation remain distinct runtime gates and cannot be inferred from this plan                                                                                         |
...
- Policy-TTY transport uses nonce-bound exact-order ASCII `BEGIN`/`CHUNK`/`END`; generation 8 uses challenge/frame v9. Limits remain line `4,096`, valid worst case `2,618`, total `102,400`, policy `1..65,536`, payload `2,304`, `N <= 38`, and one five-minute deadline. Retired v1-v8 domains, truncation, duplicate/missing/reordered/control/trailing/buffered input reject before policy, RPC, custody, or authority.
- Runtime timing caps are exactly `300/240/120/60/60/20/30` seconds for envelope, owner-entry cap, execution authority, minimum remaining at claim, maximum post-confirmation preclaim, post-recheck reserve, and freshness. The owner-entry deadline is further bounded by preserving the full 120-second authority reserve. Generation 8 binds the exact generation-7 terminal through owner-v11; only an exact match captures `confirmedAt`.
...
- Append-only recovery generation 8: generation 7 accepted owner-v10 for release `dbd4950e62b469379dc9fc877668d247b38b6f93` and durably retained claim SHA-256 `0xceec9b1e6de22bc8eb11c9f1bea3d6cec730e34e1ce8f306705fa4782c39c3bd` plus `failed_before_worker` terminal SHA-256 `0x97bb22de4f86b517af0b517f6765d77896da7881708da6589d17703790abc3dc`. There was no worker authorization/start, custody unlock, signature, submission, transaction hash, or receipt. Generation 8 binds that terminal and inherited generation-6 terminal, separately requires submission-v6 to contain no retained transaction state, rereads fourteen namespaces, and requires a fresh distinct envelope. Policy/runtime-instantiation is v8, TTY is v9, owner command/text/confirmation is v11, signing/intent/broadcast are v8, active signing is v8, and active submission-v7 uses v9 records with durable owner policy v9. Post-claim dual-RPC requests are ordered with at most one in flight per endpoint and no retry; known failures persist their exact code.
