...
| Real testnet proof           | PTA DONE / PROTOCOL ACTION BLOCKED | PTA deployment and the read-only PTA/WBNB readiness snapshot are independently reconciled. The proposed arbitrary test seed and `1,000 PTA` + `0.001 WBNB` LP caps are not approved. Pool initialization and LP mint are separate writes; the first Pancake pool/position/session operation remains blocked on fresh eligibility evidence, separate explicit approval, deployed ledgers, exact authority and the final passkey ceremony                                                                                                                                                                     |
...
- Testnet fallback asset: PTA is a verified non-economic ERC-20 with no admin or later-mint surface, not a stablecoin or execution guard. Its exact chain-97 runtime and fixed supply are independently confirmed at `0x4ed64525d6fB06b7dA926C683CBD809632C9B4Cc`. The preparation now records a fee-500 CREATE2 candidate only conditionally until the pool init-code compiler/artifact and deployer derivation are independently bound. Its intended `1 PTA = 0.000001 WBNB` seed is an arbitrary test scenario, not a market/peg/oracle/valuation claim. The factory owner and unadmitted LM-deployer surface remain mutable-control risks. No transaction envelope, pool, price, liquidity, oracle, position ownership or execution authority is established.
