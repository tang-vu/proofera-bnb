...
The Windows release topology now fails closed without an immutable build identifier. Its release probe binds the public health response to the exact published commit and requires readiness to be honestly `not_ready`, never `misconfigured`; a PM2 monitor repeats the same marketplace/agent/Card/readiness checks every five minutes. The public `/proof` route renders the exact build, four endpoint skill contracts and the committed seven-gate submission ledger, including blockers and digest-bound artifacts. Release `e6e55c161f324f03b401b1e9f0a17f5fbff2d373` remains the historical frozen release. The current UI release `ad0cee11885b2131c27bfa14c3b0a27f2f8fee69` passed the final host-origin capture and a separately authorized bounded rollback to `676ab796bec729a734355737d6c0509a27c9d1d5`; both rollback and restoration ran the exact web/monitor paths and passed all eleven checkpoints. An environment-only PM2 preflight was rejected rather than counted. External alert delivery, independent uptime evidence, deployed data/worker probes, final demo, authoritative submission receipt and a genuinely ready activation path remain open.
...
the Pancake benefit gate. The rollback/freeze gate is now verified; deployment of the evidence
carrier, exact-build final capture, clean-room playback and organizer receipt remain separate
