...
- Policy-TTY transport uses nonce-bound exact-order ASCII `BEGIN`/`CHUNK`/`END`; generation 7 uses challenge/frame v8. Limits remain line `4,096`, valid worst case `2,618`, total `102,400`, policy `1..65,536`, payload `2,304`, `N <= 38`, and one five-minute deadline. Retired v1-v7 domains, truncation, duplicate/missing/reordered/control/trailing/buffered input reject before policy, RPC, custody, or authority.
- Runtime timing caps are exactly `300/240/120/60/60/20/30` seconds for envelope, owner-entry cap, execution authority, minimum remaining at claim, maximum post-confirmation preclaim, post-recheck reserve, and freshness. The owner-entry deadline is further bounded by preserving the full 120-second authority reserve. Generation 7 binds the exact generation-6 terminal through owner-v10; only an exact match captures `confirmedAt`.
...
- Append-only recovery generation 7: generation 6 accepted owner-v9 for release `1655d39db63a636e7c66a007046c06eab65c55f1` and durably retained only claim SHA-256 `0x2f7dffbe7fef710273206009a06c7e460fa9f289b2403d6760c805707467e2ed` plus `failed_before_worker` terminal SHA-256 `0x4a64cc2ef48529e271152004e31dfb7d35511d0a5691815838849c831638d6f7`. There was no worker authorization/start, custody unlock, signature, submission, transaction hash, or receipt. Generation 7 binds that terminal and its inherited generation-5 terminal, separately requires submission-v5 to contain no retained transaction state, rereads twelve namespaces, and requires a fresh distinct envelope. Policy/runtime-instantiation is v7, TTY is v8, owner command/text/confirmation is v10, signing/intent/broadcast are v7, active signing is v7, and active submission-v6 uses v8 records with durable owner policy v8.
