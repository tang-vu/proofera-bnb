...
| Pancake read/execute adapter | ACTIVE / LP SCOPE PREPARED       | Atomic reads, source/selector/calldata validation, release pinning and QuickEdit-off bootstrap pass. Generation 10 consumed the sole initializer send; no resend/replacement exists. Both fixed providers observe the status-1 receipt/logs, while missing historical EIP-1898 state still prevents ProofEra's strict terminal confirmation. A later read-only first-LP rehearsal at common block `127358821` verified current code/state and identical approval/mint simulations without custody access, signature or broadcast; its two-minute scope is expired and cannot authorize execution.                                                                                                                                                  |
...
- First-LP exact-scope preparation: the root command `pnpm prepare:pancake:lp:testnet --capture-read-only-exact-scope --source-commit <COMMIT40>` is read-only and create-only. It requires the fixed chain-97 dual RPCs to agree on a common block, six reviewed runtime identities, zero EIP-1967 slots, exact factory/manager/token relations, seed `slot0`, zero liquidity, zero allowances, no pending nonce, balances, gas and both simulations. The retained block-`127358821` rehearsal encodes only direct PTA `approve` then direct payable Position Manager `mint`, full range `[-887270,887270]`, exact amounts `1000000000000000000000` PTA and `1000000000000000` wei, minima equal maxima, recipient/deployer EOA, and a 15-minute calldata deadline inside a two-minute scope. The artifact is expired, not reusable, and explicitly has no custody/sign/broadcast authority. A future attempt must regenerate all bytes and obtain a separate exact authorization; ambiguous submission is never retried, and a confirmed approval followed by a failed/unsubmitted mint requires a separately scoped zero-allowance cleanup.
