...
| Testnet deployer custody     | DONE locally / FUNDED TESTNET      | Dedicated address `0x997cD959798F7c925076eaeFF5855C5C2c1e5A49`; Web3 v3 keystore plus DPAPI-wrapped random password remain outside the repo. A finalized transfer funded it with `0.1 tBNB`; two official RPCs still report nonce `0`, empty code and an unused exact target. Deployment is one-shot pinned to nonce `0` and `0x4ed64525d6fB06b7dA926C683CBD809632C9B4Cc`. No signing API, deployment broadcast or deployment receipt exists |
...
None block further local implementation. The first real PTA deployment is no longer funding-blocked: a user-provided finalized transfer supplied `0.1 tBNB` without moving mainnet value into the disposable key. The remaining blockers are local and deliberate: finish/review the dedicated durable one-shot signer, recollect fresh two-provider state, validate the exact signed transaction and only then broadcast chain `97`. Release also requires the npm-account owner to confirm revocation/rotation of the registry token exposed in prior local tool output and review the account's access/publication history; the token value is not retained here. Other expected requests remain limited to 8004scan Pro quota, durable judging-host credentials, and explicit approval before any mainnet or paid action. The BNB-managed Studio trial lasts only 48 hours and is not a judging-uptime solution.
