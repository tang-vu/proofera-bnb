...
- First-LP exact-scope preparation and execution: the read-only command `pnpm prepare:pancake:lp:testnet --capture-read-only-exact-scope --source-commit <COMMIT40>` retains only create-once preparation evidence. The historical block-`127358821` scope used a two-minute lifetime and is expired. The exact write entrypoint `pnpm execute:pancake:first-lp:testnet` is restricted to clean published `main`, rebuilds a five-minute dual-RPC common-block scope, and displays a full scope/runtime/OS-nonce binding digest. Release `3c82821` accepted v3 user presence but stopped before signing on a root-URL canonicalization mismatch; its v1 journal contains only owner record SHA-256 `0x3c862be1cff75b04bb1b02cb0b62142452bdc0065a8af43451634b18738e292b`. Release `6f38f89` accepted v4 after the URL fix but stopped before signing because parallel responses were compared to a later mutable request counter; its v2 journal contains only owner record SHA-256 `0xe6b16d01826b8a28ddf392834d1f1c5117ef84df536021c67a88a61c2e042d3c`. V5 captures request-local IDs, admits journal v3 only after exact ACL/file-count/hash checks for both predecessors, and requires a same-release three-pass `pnpm rehearse:pancake:first-lp:testnet` read-only approval recheck before another ceremony. With at least a two-minute reserve, the live same-process challenge accepts the exact case-sensitive literal `CONFIRM` once as user presence; the literal does not carry scope and cannot recreate authority. Its one-use in-memory authority binds the fixed chain-97 PTA approval and direct payable Position Manager mint, one signature and at most one send for each step, with no retry/replacement/mainnet authority. Append-only current-user journal records precede custody unlock and submission; exact signed bytes stay only in that ACL journal for no-resend reconciliation, and restarts after submission start reconcile only. Approval success on a block finalized by both RPCs gates mint, and a final LP claim requires matching dual-RPC finalized transaction/receipt/log evidence plus EIP-1898 position, pool-liquidity and zero-allowance post-state. A confirmed approval followed by a failed/unsubmitted mint requires a separately scoped zero-allowance cleanup.
