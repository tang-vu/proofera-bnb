...
| Pancake read/execute adapter | ACTIVE                             | Atomic reads, canonical-block identity, context-v3 binding, source reconstruction, selector-scoped attestation modeling and direct-calldata validation pass. The initializer is locally source/runtime reviewed and its exact bytes are published and no-redirect refetched; a fixed two-RPC common-finalized EIP-1898 coordinator plus non-authorizing one-shot descriptor pass 25 focused tests. Authenticated initializer review, a durable journal, exact signer and eligible pool remain blocked                                                                                             |
