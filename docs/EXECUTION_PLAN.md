...
| Pancake read/execute adapter | ACTIVE / LP EXECUTION GATED      | Atomic reads, source/selector/calldata validation, release pinning and QuickEdit-off bootstrap pass. Generation 10 consumed the sole initializer send; no resend/replacement exists. Both fixed providers observe the status-1 receipt/logs, while missing historical EIP-1898 state still prevents ProofEra's strict terminal confirmation. A later read-only first-LP rehearsal at common block `127358821` verified current code/state and identical approval/mint simulations without custody access, signature or broadcast; its historical two-minute scope is expired. The first literal-confirmation run stopped before signing on an RPC root-URL canonicalization mismatch and retained only an exact owner record. A corrected runner can regenerate a five-minute common-block scope only after verifying that immutable predecessor, then proceed after literal `CONFIRM` against the displayed and journal-bound complete scope/runtime/OS-nonce digest, append-only pre-sign/pre-send journal records, one-use signing/send capabilities, dual-RPC finalized receipts and EIP-1898 terminal checks. The literal itself carries no scope or reusable authority. No LP receipt is claimed yet. |
| Fixed test-asset fallback    | DONE / FIRST-LP PREPARATION ONLY | Chain-97-only fixed-supply PTA and the initialized fee-500 PTA/WBNB pool retain exact code/source lineage. The expired block-`127358821` preparation binds a direct full-range zero-slippage mint of `1,000 PTA + 0.001` native BNB and expected raw liquidity `10^18`; no WBNB approval or dispatcher is allowed. It remains read-only preparation: no current authority, liquidity, NFT, receipt, oracle history or realized benefit exists.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
- Testnet fallback asset: PTA is a verified non-economic ERC-20 with no admin or later-mint surface, not a stablecoin or execution guard. Its exact chain-97 runtime and fixed supply are independently confirmed at `0x4ed64525d6fB06b7dA926C683CBD809632C9B4Cc`. Both fixed RPCs now return a matching status-1 initializer receipt whose selected normalized logs identify the reproduced fee-500 candidate. ProofEra still withholds its strict durable pool confirmation because the required historical EIP-1898 canonical/state reads are unavailable. The intended `1 PTA = 0.000001 WBNB` seed is an arbitrary test scenario, not a market/peg/oracle/valuation claim. No liquidity, oracle history, position ownership or further execution authority is established.
