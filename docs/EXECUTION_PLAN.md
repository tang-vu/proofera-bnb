...
| Testnet deployer custody     | DONE locally / UNFUNDED            | Dedicated address `0x997cD959798F7c925076eaeFF5855C5C2c1e5A49`; Web3 v3 keystore plus DPAPI-wrapped random password remain outside the repo. A chain/address/artifact/PowerShell-pinned server-only readiness probe passes 30 focused tests and 1 opt-in Windows unlock/address check; no signing API, RPC read, funding, broadcast, deployment or explorer evidence exists |
