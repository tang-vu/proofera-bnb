...
4. The general activation worker/KMS creates and retains a session private key. Separately, the bounded operator ceremony has one concrete Windows DPAPI signer whose tracked configuration contains only its public descriptor; this narrower path does not satisfy the production bootstrap/database handoff.
5. Passkey create/recover is available only at the configured final HTTPS origin and is not a grant. The production browser grant boundary still requires the exact persisted `grant_ready -> grant_submitting` transition, a production WebAuthn P-256 signer whose explicit RP ID matches the configured domain, and an atomic one-shot PostgreSQL submission claim. The bounded operator proof path is now wired separately: it stores a device-local submitting fence before SDK invocation, captures only public relay IDs/transaction hashes, and blocks an ambiguous retry while that browser state remains. Clearing site data is not a production replay guarantee, so this path cannot replace the canonical ledger. Both paths keep the session private key out of the browser.
6. The bounded worker requires two RPCs to agree that both KeyStore validity and the derived account key hash are present, reads the onchain expiry, simulates the fixed zero approval, writes a create-only execution claim, then submits with SDK `noWait: true` so a returned `callsId` can be reconciled without replacement. Completion after revoke requires both KeyStore invalidity and account-key absence; an expiry-only mismatch blocks rather than masquerading as revoke evidence. The general activation worker must additionally compare the complete production policy/bootstrap binding.
