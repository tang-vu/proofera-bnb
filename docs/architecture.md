...
- A bounded exact-lineage search rejected all 14 reviewed WBNB pools. Exact WBNB source/runtime/control provenance is now separately reproducible, closing that token-component gate for a prospective PTA pair. The search is not an exhaustive factory-lifetime proof, and the WBNB result does not waive a fresh exact-block code binding or PTA deployment, pool lineage, oracle, liquidity, mutable-control, position-ownership, and authority review for a new fixture.
