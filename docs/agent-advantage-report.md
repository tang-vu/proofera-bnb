...
Before the runs, that schema could not honestly represent unbound judged blocks, accounts, positions, policies, release commits, identities and declarations. Encoding placeholders as final `BenchmarkDeclaration` values would have misrepresented them as exact inputs. `BenchmarkPreregistrationSchema` therefore preserved the pre-run protocol until those fields were bound:
...
Retained create-only agent command (a new run requires a new preregistered run ID):
...
Retained create-only manual command (it cannot overwrite the existing capture):
...
The active declaration is [`38046f87b87c-126555555.json`](../evidence/termix/declarations/permission-audit/38046f87b87c-126555555.json), declaration SHA-256 `6f568c25eb405b23ec5ba68f72c61bfca68d63b4f7cd63dea93ff2f9a0ebc370`. Its canonical input bundle is [`38046f87b87c-126543819.canonical-json`](../evidence/termix/frozen/permission-audit/38046f87b87c-126543819.canonical-json), SHA-256 `c50a2defc62a996cab8a8bf51be2b8b2bbe44cc007ea01e6d1512d7257a8f0cb`. It binds the real lifecycle, exact-block PTA runtime, host-local PostgreSQL deployment receipt with zero matching submission claims, local proof-worker public state, pinned SDK evidence, blind corpus, Agent `1825`, verified hire and reviewer-held answer-key digest. No secret or signer material enters the bundle.

Available implementation boundary: `@proofera/benchmarks` now includes a strict deterministic audit engine for the canonical secret-free bundle. It requires distinct role-bound proposal, corpus, lifecycle, code, SDK and database-deployment evidence; ordered distinct grant/execute/revoke receipts; exact code-policy parity; and complete evidence joins. It rejects duplicate/unbound cases, compares direct targets/selectors/code/recipient/token, chain, expiry, quote age, spend bounds, retry, revoke and signer-exposure markers, and emits a corrected three-layer enforcement table. A separate fixed read-only RPC plan admits only chain ID, the named lifecycle receipt reads, and exact-block runtime-code reads; raw responses must match chain 97, successful transaction/block joins and attested code bytes. The fixed manual lane additionally requires positive timed work, all artifact digests exactly once, every fixed RPC observation, all five preregistered declaration-input joins, no hire receipt and a canonical output with `agentInvoked: false`. The fixed agent lane cannot reach any network until the outer runner validates a registered ERC-8004 identity and verified hire receipt; it then completes the same RPC plan before one fixed A2A skill and accepts only an exact-bundle, explicitly non-executing output. The v1.1 skill is deployed on the public LP endpoint; 21 agent tests/build/audit pass, and a 2026-08-22 local/public smoke on the frozen bundle produced byte-equivalent canonical output with `executionPerformed: false`. That smoke is availability/parity evidence only, not a TermiX run, transcript, receipt or performance evidence.
...
gate. These runner capabilities produced the valid v2 raw captures and observational pair;
independent review and final verified compilation remain separate gates.

Retained create-only commands; any new execution requires a new preregistered run ID and exact invocation rather than overwriting the existing captures:
