...
The chain-97 first-LP preparer is a narrower execution-preparation layer, not a replacement for that generic context. It admits only the fixed PTA/WBNB fee-500 empty pool, two exact official RPCs, the reviewed runtime/proxy identities, full-range aligned ticks, `1,000 PTA`, `0.001` native BNB, and zero slippage. It encodes a direct exact PTA approval followed by direct payable `mint`; equal minima/maxima make any changed token consumption revert and avoid a successful native-dust outcome. Both calls are simulated independently on each RPC, with the mint simulation using an ephemeral PTA allowance state override whose storage slot is source/code-hash bound. The override changes no chain state. The retained rehearsal used a 120-second scope and is expired. Fresh execution scopes expire after 300 seconds and still carry no signing/broadcast authority. The live same-process challenge separately persists the complete scope/runtime/OS-nonce binding and accepts exact literal `CONFIRM` once as TTY user presence; the literal itself carries no scope or reusable authority. Exact RPC origins are compared after standards-compliant root-path canonicalization without accepting redirects, each parallel response is bound to its request-local ID, and a fresh journal generation requires exact immutable predecessor audits. The bounded custody probe exercises the same runtime/path/ACL/hash/DPAPI/keystore/postcheck sequence as production but stops and clears its owned scalar before signature construction; production constructs a signature only after that whole sequence and a current deadline. The same-release three-pass approval rehearsal uses the production RPC read checks without journal creation, custody, signing or broadcast; mint remains covered separately by the preparer's two-provider state-override simulation. Neither probe nor scope can establish a receipt, NFT, liquidity or benefit.
